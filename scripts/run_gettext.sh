...
do
    ...
    if [ -f "$dir/OrcaSlicer_${lang}.po" ]; then
        ...
        mo_out="resources/i18n/${lang}/OrcaSlicer.mo"
        if ! msgfmt --check-format -o "$mo_out" "$dir/OrcaSlicer_${lang}.po"; then
            echo "Error encountered with msgfmt command for language ${lang}."
            exit 1  # Exit the script with an error status
        fi
        # Verslicer uses SLIC3R_APP_KEY "verslicer" (and legacy "Verslicer") for wx catalogs.
        cp -f "$mo_out" "resources/i18n/${lang}/verslicer.mo"
        cp -f "$mo_out" "resources/i18n/${lang}/Verslicer.mo" || true
    fi
done
