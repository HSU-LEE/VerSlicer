...
DEFAULT_APP="${PROJECT_DIR}/build/${ARCH}/dist/VerSlicer.app"
FALLBACK_APP="${PROJECT_DIR}/build/${ARCH}/OrcaSlicer/Verslicer.app"
APP_PATH="${1:-${DEFAULT_APP}}"
OUT_DMG="${2:-}"

if [ ! -d "${APP_PATH}" ] && [ -d "${FALLBACK_APP}" ]; then
    echo "note: using fallback app path ${FALLBACK_APP}"
...
if [ ! -d "${APP_PATH}" ]; then
    ...
    echo "Then: ./scripts/make_macos_installer.sh" >&2
    exit 1
fi

# Dev builds symlink Resources; materialize before packaging.
if [ -L "${APP_PATH}/Contents/Resources" ]; then
    echo "App bundle still has symlinked Resources — preparing portable copy..."
    PREPARED="${PROJECT_DIR}/build/${ARCH}/dist/.package_staging_$$/VerSlicer.app"
    mkdir -p "$(dirname "${PREPARED}")"
    "${SCRIPT_DIR}/prepare_macos_app_bundle.sh" "${APP_PATH}" "${PREPARED}"
    APP_PATH="${PREPARED}"
    APP_NAME="VerSlicer"
fi
...
System requirements
  • macOS 11.3 or later
  • Apple Silicon (${ARCH}) build
  • Internet for MakerWorld / cloud features (optional)

Support
  • https://github.com/SoftFever/OrcaSlicer (upstream)
EOF
