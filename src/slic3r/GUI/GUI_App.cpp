...
public:
    SplashScreen(wxPoint pos = wxDefaultPosition)
        ...
        : wxSplashScreen(wxBitmap(FromDIP(wxSize(360, 260), nullptr)), wxSPLASH_CENTRE_ON_SCREEN, 0, nullptr, wxID_ANY, wxDefaultPosition, wxDefaultSize,
#ifdef __APPLE__
        ...
        )
    {
        ...
        const std::string splash_key = dark_mode ? "splash_logo_dark" : "splash_logo";
        // Logo ~48% of window width, centered — avoid filling the whole splash with a scaled bitmap.
        const int logo_px = std::max(FromDIP(88), sz.GetWidth() * 30 / 100);
        wxBitmap *splash_bmp = bmp_cache.load_png(splash_key, logo_px, logo_px);
        if (splash_bmp) {
            m_logo_bmp = *splash_bmp;
            m_logo_pos = wxPoint((sz.GetWidth() - m_logo_bmp.GetWidth()) / 2,
                                 std::max(FromDIP(20), sz.GetHeight() / 10));
        }

        m_font_title = Label::Head_24;
        m_title_color = StateColor::darkModeColorFor(wxColour("#FF8F4A"));
    ...
    void OnPaint(wxPaintEvent& evt)
    {
        ...
        dc.Clear();
        if (m_logo_bmp.IsOk())
            dc.DrawBitmap(m_logo_bmp, m_logo_pos.x, m_logo_pos.y, true);

        wxRect rc = wxRect(0, 0, c_sz.GetWidth(), 0);
        if (m_logo_bmp.IsOk()) {
            dc.SetFont(m_font_title);
            dc.SetTextForeground(m_title_color);
            rc.y = m_logo_pos.y + m_logo_bmp.GetHeight() + FromDIP(8);
            rc.height = dc.GetTextExtent(m_title_text).GetHeight();
            dc.DrawLabel(m_title_text, rc, wxALIGN_CENTER);
        }

        dc.SetTextForeground(m_fg_color);
        dc.SetFont(m_font_action);
        rc.y      = c_sz.GetHeight() * 0.82;
...
private:
    ...
    wxString m_title_text   = wxString::FromUTF8(SLIC3R_APP_FULL_NAME);
    wxString m_text_action  = _L("Loading configuration") + dots;

    wxFont m_font_title   = Label::Head_32;
...
static void migrate_flatpak_legacy_datadir(const boost::filesystem::path &data_dir_path)
{
    ...
    boost::replace_first(legacy_data_dir_str, "com.orcaslicer.Verslicer", "io.github.orcaslicer.Verslicer");
...
/*#ifdef WIN32
    ...
    try {
        ...
        wxString errmsg = wxString::Format(_L("%s will terminate because of running out of memory. "
                                              "It may be a bug. It will be appreciated if you report the issue to our team."), SLIC3R_APP_FULL_NAME);
        ...
        wxString errmsg = wxString::Format(_L("%s will terminate because of a localization error. "
                             "It will be appreciated if you report the specific scenario this issue happened."), SLIC3R_APP_FULL_NAME);
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        wxLogError(format_wxstr(_L("%1% got an unhandled exception: %2%"), SLIC3R_APP_FULL_NAME, ex.what()));
        throw;
    }
//#endif
}
...
//#endif
    ...
    if (m_networking_need_update) {
        if (std::string(SLIC3R_APP_FULL_NAME) == "Verslicer") {
            CallAfter([this]() {
                if (!ensure_bambu_network_plugin_installed(true))
                    show_network_plugin_download_dialog(false);
            });
        } else {
            show_network_plugin_download_dialog(false);
        }
    } else if (std::string(SLIC3R_APP_FULL_NAME) == "Verslicer"
        && app_config->get_bool("installed_networking")
        && !Slic3r::NetworkAgent::is_network_module_loaded()) {
        CallAfter([this]() { ensure_bambu_network_plugin_installed(true); });
    }
    ...
    Slic3r::GUI::BambuSmartPrintService::instance().initialize();
...
{
	//app config initializes early becasuse it is used in instance checking in Verslicer.cpp
...
void GUI_App::cleanup_ollama_assistant()
{
    OllamaClient::cancel_active_requests(OllamaCancelDomain::All);

    if (ollama_voice_input) {
        ollama_voice_input->stop();
        ollama_voice_input.reset();
    }

    OllamaServerManager::shutdown_if_started();

    OllamaChatDialog* dlg = ollama_chat_dialog;
    ollama_chat_dialog    = nullptr;
...
bool GUI_App::ensure_bambu_network_plugin_installed(bool offer_download)
{
    if (!app_config)
        return false;

    if (Slic3r::NetworkAgent::is_network_module_loaded() && m_agent)
        return true;

    std::string ver = app_config->get_network_plugin_version();
    if (ver.empty()) {
        ver = get_latest_network_version();
        app_config->set_network_plugin_version(ver);
        app_config->save();
    }

    auto plugin_present = [&]() {
        const std::string v = app_config->get_network_plugin_version();
        return BBLNetworkPlugin::versioned_library_exists(v) || BBLNetworkPlugin::legacy_library_exists();
    };

    if (!plugin_present() && offer_download) {
        const std::string latest = get_latest_network_version();
        if (app_config->get_network_plugin_version() != latest) {
    ...
    if (!Slic3r::NetworkAgent::is_network_module_loaded()) {
        ...
        hot_reload_network_plugin();
    }

    return Slic3r::NetworkAgent::is_network_module_loaded() && m_agent;
}

void GUI_App::notify_bambu_network_plugin_required()
{
    ...
    const wxString msg = _L("Printer control failed: Bambu network plug-in is not loaded (error -1).\n\n"
                            "Wait for the download to finish at startup, or open Preferences → Online to install it, then restart.");
    ...
    CallAfter([this, msg]() {
        if (plater() && plater()->get_notification_manager()) {
            ...
            MessageDialog dlg(mainframe, msg, _L("Network plug-in required"), wxOK | wxICON_WARNING);
            dlg.ShowModal();
        }
        ensure_bambu_network_plugin_installed(true);
    });
}
        ...
        m_agent->set_on_printer_connected_fn([this](std::string dev_id) {
            ...
            GUI::wxGetApp().CallAfter([this, dev_id] {
                ...
                if (obj) {
                    ...
                    // Keep user-entered LAN access code; cloud fetch overwrites it for cloud printers only.
                        ...
                        if (obj->is_lan_mode_printer()) {
                            if (state == ConnectStatus::ConnectStatusOk) {
                                ...
                                if (msg == "5") {
                                    ...
                                    wxString code_hint = from_u8(msg);
                                    if (msg.empty() || msg == dev_id || msg == "unsigned_studio") {
                                        code_hint = _L("Check printer IP, LAN access code, and that Developer mode is ON");
                                    }
                                    text = wxString::Format(
                                        _L("Connect %s failed (SN:%s).\n\n%s"),
                                        from_u8(obj->get_dev_name()), obj->get_dev_id(), code_hint);
                                    wxGetApp().show_dialog(text);
                                    show_ip_address_enter_dialog(
                                        _L("Enter printer IP and LAN access code (shown on the printer screen)"));
                                }
                                event.SetInt(-1);
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("Verslicer ");
    if (start == std::string::npos) {
        start = body.find("Verslicer ");
        if (start == std::string::npos)
            return boost::none;
    }
...
void GUI_App::init_webview_runtime()
{
    ...
    if (!WebView::CheckWebViewRuntime()) {
        int nRet = wxMessageBox(_L("Verslicer requires the Microsoft WebView2 Runtime to operate certain features.\nClick Yes to install it now."),
...
//  SetAppName(SLIC3R_APP_KEY "-beta");
    ...
    // Verslicer requires Bambu network plugin + cloud (unlike stock Orca where these are optional).
    if (std::string(SLIC3R_APP_FULL_NAME) == "Verslicer") {
        bool changed = false;
        if (!app_config->get_bool("installed_networking")) {
            app_config->set_bool("installed_networking", true);
            changed = true;
            BOOST_LOG_TRIVIAL(info) << "Verslicer: enabled installed_networking";
        }
        if (!app_config->has_cloud_provider(BBL_CLOUD_PROVIDER)) {
            app_config->add_cloud_provider(BBL_CLOUD_PROVIDER);
            changed = true;
            BOOST_LOG_TRIVIAL(info) << "Verslicer: enabled Bambu cloud provider";
        }
        if (app_config->get_network_plugin_version().empty()) {
            app_config->set_network_plugin_version(get_latest_network_version());
            changed = true;
            BOOST_LOG_TRIVIAL(info) << "Verslicer: defaulted network_plugin_version";
        }
        if (changed)
            app_config->save();
    }
...
{
    // Same policy as Verslicer (Verslicer/Verslicer): legacy plugin needs a pinned version.
    if (BBLNetworkPlugin::instance().get_get_my_token() == nullptr) {
        return "01.10.01.50";
    }
    // Report Orca-compatible API version to Bambu cloud (no separate auth key exists in Orca).
    return VersionInfo::convert_full_version(SoftFever_VERSION);
}
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
    ...
    if (is_editor()) {
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    wxString::Format(_L("%s needs an update"), SLIC3R_APP_FULL_NAME),
                    false,
                    wxCENTER | wxICON_INFORMATION);
...
//     update_mode(); // !!! do that later
    ...
    mainframe->Show(true);
#ifndef __APPLE__
    mainframe->SetTitle(SLIC3R_APP_FULL_NAME);
    if (mainframe->topbar())
        mainframe->topbar()->SetTitle(SLIC3R_APP_FULL_NAME);
#endif
...
//#endif
    ...
    if (m_config_corrupted) {
        ...
        show_error(nullptr,
                   _u8L(
                       "The configuration file may be corrupted and cannot be parsed.\nThe application has attempted to recreate the "
...
bool GUI_App::on_init_network(bool try_backup)
{
    ...
    if (should_load_networking_plugin) {
        if (config_version.empty()) {
            ...
            app_config->set_network_plugin_version(config_version);
            app_config->save();
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": defaulted network_plugin_version to " << config_version;
        }

        const bool plugin_on_disk = BBLNetworkPlugin::versioned_library_exists(config_version)
                                 || BBLNetworkPlugin::legacy_library_exists();
        if (!plugin_on_disk) {
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ": plugin DLL missing for version " << config_version;
...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #FF8F4A  -- 73
        if (bg_col != orig_col) {
        ...
        if (command.has_value()) {
            ...
            else if (command_str.compare("makerworld_model_open") == 0) 
            {
                if (root.get_child_optional("model") != boost::none) {
                    ...
                    boost::optional<std::string> path      = data_node.get_optional<std::string>("url");
                    if (path.has_value()) {
                        const std::string raw = url_decode(path.value());
                        wxString          realurl = from_u8(raw);
                        if (is_makerworld_host_url(raw) || raw.find(".3mf") != std::string::npos) {
                            const std::string info = MakerWorldSearchService::resolve_download_info(raw);
                            if (!info.empty())
                                realurl = from_u8(info);
                        }
...
void GUI_App::on_http_error(wxCommandEvent &evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(nullptr, _L("The version of Verslicer is too low and needs to be updated to the latest version before it can be used normally."), "", wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {
        }
    }
...
bool GUI_App::process_network_msg(std::string dev_id, std::string msg)
{
    if (dev_id.empty()) {
        if (msg == "wait_info") {
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, wxString::Format(_L("Please try updating %s and then try again."), SLIC3R_APP_FULL_NAME), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, wxString::Format(_L("The certificate has expired. Please check the time settings or update %s and try again."), SLIC3R_APP_FULL_NAME), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, wxString::Format(_L("Internal error. Please try upgrading the firmware and %s version. If the issue persists, contact support."), SLIC3R_APP_FULL_NAME), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            static bool unsigned_studio_shown = false;
            if (!unsigned_studio_shown) {
                unsigned_studio_shown = true;
                MessageDialog msg_dlg(nullptr,
                    wxString::Format(_L("To use %s with Bambu Lab printers, enable LAN mode and Developer mode on the printer.\n\n"
                       "1. Printer settings → turn on LAN mode\n"
                       "2. Enable Developer mode (required for third-party slicers)\n"
                       "3. In Verslicer Device tab, set the printer IP and LAN access code\n\n"
                       "Cloud print from non-Bambu Studio builds is blocked (error -26); LAN is required."),
            ...
            if (m_agent) {
                MachineObject* obj = m_device_manager ? m_device_manager->get_my_machine(dev_id) : nullptr;
                if (!obj && m_device_manager)
                    obj = m_device_manager->get_selected_machine();
                if (obj)
                    m_agent->install_device_cert(obj->get_dev_id(), obj->is_lan_mode_printer());
            }
            CallAfter([this]() {
                show_ip_address_enter_dialog(_L("LAN + Developer mode required — enter IP and access code"));
            });
            return true;
        }
    }
...
// Check if the user's Verslicer profile directory is empty and offer to migrate
...
bool GUI_App::maybe_migrate_user_presets_on_login()
{
    ...
    BOOST_LOG_TRIVIAL(info) << "Migrate user presets to the Verslicer user folder if needed.";
    ...
    // Check if the user already has presets on Verslicer.
    ...
    // that haven't been synced down yet (e.g. fresh install with existing cloud account).
    {
        ...
        if (ret == 0 && !cloud_presets.empty()) {
            BOOST_LOG_TRIVIAL(info) << "Verslicer already has " << cloud_presets.size()
                                    << " presets, skipping migration for user: " << new_user_id;
            return false;
        }
        if (ret != 0) {
            BOOST_LOG_TRIVIAL(warning) << "Failed to query Verslicer presets (error " << ret
                                       << "), skipping migration to avoid overwriting cloud data.";
            return false;
        }
        BOOST_LOG_TRIVIAL(info) << "Verslicer has no presets for user " << new_user_id << ", proceeding with migration check.";
    }
    ...
    wxString msg = wxString::Format(
        _L("Existing user presets were found in %s.\n"
           "Do you want to migrate them to your Verslicer profile?\n"
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by Verslicer.conf: %1%") % language;
        else {
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by Verslicer: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
    ...
    if (!wxLocale::IsAvailable(locale_language_info->Language)) {
	    ...
	    wxString message = "Switching Verslicer to language " + requested_language_code + " failed.";
        ...
        wxMessageBox(message, "Verslicer - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
    ...
    // MO files are generated as OrcaSlicer.mo; aliases verslicer.mo / Verslicer.mo are copied in run_gettext.sh.
    m_wxLocale->AddCatalog("OrcaSlicer");
    if (std::string(SLIC3R_APP_KEY) != "Verslicer")
        m_wxLocale->AddCatalog("Verslicer");
        ...
        if (selection_data_arr.size() == 2) {
            ...
            if (!ip_address.empty()) {
                ...
                if (obj->is_lan_mode_printer()) {
                    ...
                    if (dm)
                        dm->set_selected_machine(obj->get_dev_id());
                }
            }
        }
    });
...
void GUI_App::open_smart_print()
{
    if (!mainframe)
        return;
    mainframe->jump_to_smart_print();
    if (plater_)
        plater_->get_current_canvas3D()->force_set_focus();
}
...
void GUI_App::load_current_presets(bool active_preset_combox/*= false*/, bool check_printer_presets_ /*= true*/)
{
    ...
    BambuSmartPrintService::instance().on_app_preset_context_changed();
}
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto Verslicer icon in Finder,
...
bool GUI_App::run_wizard(ConfigWizard::RunReason reason, ConfigWizard::StartPage start_page)
{
    ...
    if (res) {
        ...
        update_publish_status();
        mainframe->refresh_plugin_tips();
        GUI::BambuSmartPrintService::on_first_guide_completed();
        // BBS: remove SLA related message
    }
...
{
    if (app_config->get("firstguide", "finish") == "1")
        return false;
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Orca.Slicer.1";
    std::wstring prog_desc = L"Verslicer";
