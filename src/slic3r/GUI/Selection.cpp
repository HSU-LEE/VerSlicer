...
void notify_ai_assistant_selection_changed(const std::set<unsigned int>& current)
{
    static std::set<unsigned int> previous;
    if (previous == current)
        return;
    previous = current;
    if (wxGetApp().plater())
        OllamaActionExecutor::notify_plater_context_changed(false);
}
...
{
    notify_ai_assistant_selection_changed(m_list);
