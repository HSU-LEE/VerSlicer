...
wxString hide_id_middle_string(wxString const &str, size_t offset, size_t length);
wxString hide_passwd(wxString url, std::vector<wxString> const &passwords);

bool MediaPlayCtrl::can_use_lan_liveview() const
{
    return !m_disable_lan && m_lan_proto > MachineObject::LVL_Disable && !m_lan_ip.empty();
}

bool MediaPlayCtrl::prefer_lan_liveview() const
{
    if (!can_use_lan_liveview())
        return false;
    if (m_lan_mode || !m_remote_proto)
        return true;
    // Printer reports cloud liveview unavailable — use LAN RTSP/local tunnel.
    if (m_tutk_state == "disable" || boost::algorithm::icontains(m_tutk_state, "disable"))
        return true;
    // Online printers: prefer LAN when RTSP/rtsps is available (cloud 403 / auth issues).
    return m_lan_proto == MachineObject::LVL_Rtsp
        || m_lan_proto == MachineObject::LVL_Rtsps
        || m_lan_proto == MachineObject::LVL_Local;
}

wxString MediaPlayCtrl::liveview_forbidden_user_message()
{
    return _L("Cloud liveview was denied (HTTP 403 — forbidden). This usually means the server "
              "reached you but rejected the request, not a general network outage.\n\n"
              "Try:\n"
              "• Sign out of your Bambu Lab account, quit SlicePilot completely, then sign in again\n"
              "• Preferences → Login → check China / Global region matches your account\n"
              "• Preferences → Online → update the network plug-in\n"
              "• Same Wi‑Fi as the printer, or enter the printer IP for LAN liveview\n"
              "• Temporarily disable VPN / ad blockers\n\n"
              "Smart Print does not handle camera auth; liveview uses the Bambu network plug-in only.");
}

bool MediaPlayCtrl::try_lan_fallback(const std::string& agent_version, const wxString& failure_hint)
{
    if (!can_use_lan_liveview())
        return false;
    BOOST_LOG_TRIVIAL(info) << "MediaPlayCtrl: trying LAN liveview fallback";
    m_last_state  = MEDIASTATE_IDLE;
    m_disable_lan = false;
    m_failed_code = 0;
    if (start_lan_play(agent_version))
        return true;
    wxString msg = _L("Connection Failed. Please check the network and try again");
    if (!failure_hint.IsEmpty())
        Stop(msg, failure_hint);
    else
        Stop(msg, _L("Cloud liveview unavailable and LAN stream could not start."));
    return true;
}

bool MediaPlayCtrl::start_lan_play(const std::string& agent_version)
{
    if (!prefer_lan_liveview())
        return false;
    ...
    {
        const wxString log_url = hide_passwd(
            hide_id_middle_string(from_u8(url), url.find(m_lan_ip), m_lan_ip.length()),
            {from_u8(m_lan_passwd)});
        BOOST_LOG_TRIVIAL(info) << "MediaPlayCtrl LAN: " << into_u8(log_url);
    }
    m_url = url;
    load();
...
void MediaPlayCtrl::SetMachineObject(MachineObject* obj)
{
    ...
    if (obj) {
        ...
        if (m_lan_ip.empty() && wxGetApp().app_config)
            m_lan_ip = wxGetApp().app_config->get("ip_address", machine);
        if (m_lan_proto <= MachineObject::LVL_None && !obj->local_rtsp_url.empty()) {
            m_lan_proto = obj->local_rtsp_url == "disable" ? MachineObject::LVL_Disable
                : boost::algorithm::starts_with(obj->local_rtsp_url, "rtsps") ? MachineObject::LVL_Rtsps
                : MachineObject::LVL_Rtsp;
        }
        // Online printers often report liveview local "none" until full status; allow LAN if we have IP + code.
        if (m_lan_proto <= MachineObject::LVL_None && !m_lan_ip.empty() && !obj->get_access_code().empty())
            m_lan_proto = MachineObject::LVL_Rtsps;
        m_lan_passwd     = obj->get_access_code();
        if (m_lan_passwd.empty())
            m_lan_passwd = "bblp";
...
void MediaPlayCtrl::Play()
{
    ...
    BOOST_LOG_TRIVIAL(info) << "MediaPlayCtrl::Play: lan_proto=" << m_lan_proto
                            << " remote=" << m_remote_proto << " disable_lan=" << m_disable_lan
                            << " tutk=" << m_tutk_state;
    NetworkAgent *agent = wxGetApp().getAgent();
    std::string  agent_version = agent ? agent->get_version() : "";
    if (prefer_lan_liveview()) {
        m_disable_lan = m_remote_proto && !m_lan_mode && m_tutk_state != "disable"; // try remote next time if allowed
        if (start_lan_play(agent_version))
            return;
    }
    ...
    if (!agent) {
        Stop(_L("Network plugin is not available. Enable it under Preferences → Online."));
        return;
    }

    if (!m_lan_mode && !can_use_lan_liveview()
        && !agent->is_user_login(BBL_CLOUD_PROVIDER)) {
        Stop(_L("Sign in to your Bambu Lab account to use cloud liveview, or set the printer IP for LAN mode."));
        return;
    }

    m_label_stat->SetLabel({});
    SetStatus(_L("Initializing..."));
    begin_cloud_play(agent_version);
}

void MediaPlayCtrl::begin_cloud_play(const std::string& agent_version)
{
    m_pending_cloud_protos.clear();
    auto push_unique = [this](int p) {
        if (p < 0 || p > 3)
            return;
        if (std::find(m_pending_cloud_protos.begin(), m_pending_cloud_protos.end(), p) == m_pending_cloud_protos.end())
            m_pending_cloud_protos.push_back(p);
    };
    push_unique(m_remote_proto);
    for (int p : {1, 2, 3, 0})
        push_unique(p);
    request_next_cloud_play(agent_version);
}

void MediaPlayCtrl::request_next_cloud_play(const std::string& agent_version)
{
    NetworkAgent* agent = wxGetApp().getAgent();
    if (!agent) {
        Stop(_L("Network plugin is not available. Enable it under Preferences → Online."));
    ...
    if (m_pending_cloud_protos.empty()) {
        if (try_lan_fallback(agent_version))
            return;
        Stop(_L("Liveview connection failed"),
             m_failed_code == 403 ? liveview_forbidden_user_message() : wxString{});
        return;
    }

    const int proto = m_pending_cloud_protos.front();
    m_pending_cloud_protos.erase(m_pending_cloud_protos.begin());

    static const char* protocols[] = {"", "\"tutk\"", "\"agora\"", "\"tutk\",\"agora\""};
    m_last_state  = MEDIASTATE_INITIALIZING;
    m_failed_code = 0;
    m_button_play->SetIcon("media_stop");

    BOOST_LOG_TRIVIAL(info) << "MediaPlayCtrl: cloud liveview proto=" << proto
                            << " remaining=" << m_pending_cloud_protos.size();

    agent->get_camera_url(m_machine + "|" + m_dev_ver + "|" + protocols[proto],
        [this, m = m_machine, v = agent_version, dv = m_dev_ver, proto,
        ...
        CallAfter([this, m, url, av = v, proto]() {
            if (m != m_machine) {
            ...
            if (boost::ends_with(url, "]")) {
                ...
                if (n != std::string::npos) {
                    const std::string code = url.substr(n + 1, url.length() - n - 2);
                    const size_t colon = code.find(':');
                    m_failed_code = colon != std::string::npos
                        ? std::atoi(code.substr(colon + 1).c_str())
                        : std::atoi(code.c_str());
                }
            }
            const bool remote_forbidden = m_failed_code == 403
                || url.find("403") != std::string::npos
                || boost::algorithm::icontains(url, "forbidden");

            BOOST_LOG_TRIVIAL(warning) << "MediaPlayCtrl: cloud liveview failed proto=" << proto
                                     << " code=" << m_failed_code << " url=" << url;

            if (remote_forbidden && !m_pending_cloud_protos.empty()) {
                m_last_state = MEDIASTATE_IDLE;
                request_next_cloud_play(av);
                return;
            }

            if (remote_forbidden || m_tutk_state == "disable"
                || boost::algorithm::icontains(m_tutk_state, "disable")) {
                m_last_state = MEDIASTATE_IDLE;
                if (try_lan_fallback(av, from_u8(url)))
                    return;
                if (remote_forbidden && m_lan_ip.empty()
                    && wxGetApp().show_modal_ip_address_enter_dialog(
                        false, _L("Cloud liveview denied (403). Enter the printer IP to use LAN liveview."))) {
                    if (MachineObject* obj = wxGetApp().getDeviceManager()
                            ? wxGetApp().getDeviceManager()->get_selected_machine() : nullptr) {
                        m_lan_ip = obj->get_dev_ip();
                        if (m_lan_ip.empty() && wxGetApp().app_config)
                            m_lan_ip = wxGetApp().app_config->get("ip_address", m_machine);
                        if (m_lan_proto <= MachineObject::LVL_None)
                            m_lan_proto = MachineObject::LVL_Rtsps;
                    }
                    m_failed_retry = 0;
                ...
                if (remote_forbidden) {
                    Stop(_L("Liveview access denied (403)"), liveview_forbidden_user_message());
                    return;
                }
            }
