...
std::tuple<wxBoxSizer*, ComboBox*> PreferencesDialog::create_item_combobox_base(wxString title, wxString tooltip, std::string param, std::vector<wxString> vlist, unsigned int current_index)
{
    ...
    if (current_index >= 0 && current_index < (int) combobox->GetCount())
        combobox->SetValue(combobox->GetString((unsigned) current_index));
...
static void merge_translation_domains(wxArrayString& merged, const char* domain)
{
    if (!domain || !*domain)
        return;
    const wxArrayString found = wxTranslations::Get()->GetAvailableTranslations(wxString::FromUTF8(domain));
    for (const wxString& code : found) {
        if (merged.Index(code) == wxNOT_FOUND)
            merged.Add(code);
    }
}
...
wxBoxSizer *PreferencesDialog::create_item_language_combobox(wxString title, wxString tooltip)
{
    ...
    merge_translation_domains(translations, SLIC3R_APP_KEY);
    merge_translation_domains(translations, "Verslicer");
    merge_translation_domains(translations, "OrcaSlicer");
    ...
    choice_labels.Alloc(vlist.size());
    for (size_t i = 0; i < vlist.size(); ++i) {
        ...
        if (app_config->get(param) == vlist[i]->CanonicalName) {
            m_current_language_selected = int(i);
        }
        choice_labels.Add(language_name);
    }
    if (m_current_language_selected == -1 && language.size() >= 2) {
        const std::string lang_prefix = language.size() >= 2 ? language.substr(0, 2) : language;
        for (size_t i = 0; i < vlist.size(); ++i) {
            if (vlist[i]->CanonicalName.StartsWith(wxString::FromUTF8(lang_prefix))) {
                m_current_language_selected = int(i);
                break;
            }
        }
    ...
    auto* choice = new wxChoice(m_parent, wxID_ANY, wxDefaultPosition,
                                wxSize(DESIGN_LARGE_COMBOBOX_SIZE.x, FromDIP(28)), choice_labels);
    choice->SetMinSize(wxSize(DESIGN_LARGE_COMBOBOX_SIZE.x, FromDIP(28)));
    choice->SetFont(::Label::Body_14);
    if (m_current_language_selected >= 0 && m_current_language_selected < (int) choice->GetCount())
        choice->SetSelection(m_current_language_selected);
    wxGetApp().UpdateDarkUI(choice);

    m_sizer_combox->Add(choice, 0, wxALIGN_CENTER | wxLEFT, FromDIP(5));

    choice->Bind(wxEVT_CHOICE, [this, param, vlist, choice](wxCommandEvent &e) {
        const int sel = choice->GetSelection();
        if (sel == m_current_language_selected)
            return;

        if (sel >= 0 && sel < (int) vlist.size() &&
            vlist[sel]->CanonicalName.ToStdString() != app_config->get(param)) {
            {
                ...
                if (wxGetApp().plater()->is_project_dirty()) {
                    auto result = MessageDialog(static_cast<wxWindow*>(this),
                        ...
                        wxYES_NO | wxCANCEL | wxYES_DEFAULT | wxCENTRE | wxICON_QUESTION).ShowModal();
            ...
            m_current_language_selected = sel;
...
wxBoxSizer *PreferencesDialog::create_item_checkbox(wxString title, wxString tooltip, std::string param, const wxString secondary_title)
{
    ...
    checkbox->Bind(wxEVT_TOGGLEBUTTON, [this, checkbox, param](wxCommandEvent &e) {
        ...
        if (param == SlicePilotSimpleLayout::kConfigKey) {
            SlicePilotSimpleLayout::apply(wxGetApp().mainframe);
        }
...
wxBoxSizer* PreferencesDialog::create_item_link_association( wxString url_prefix, wxString website_name)
{
    ...
    wxString tooltip = _L("Associate") + " " + url_prefix + ":// "
        + wxString::Format(_L("with %s so that it can open models from"), SLIC3R_APP_FULL_NAME) + " " + website_name;
...
void PreferencesDialog::create_items()
{
    ...
    auto item_single_instance  = create_item_checkbox(
            wxString::Format(_L("Allow only one %s instance"), SLIC3R_APP_FULL_NAME),
    #if __APPLE__
    ...
    #else
            wxString::Format(_L("If this is enabled, when starting %s and another instance is already running, that instance will be reactivated instead."),
    ...
    auto item_remember_printer = create_item_checkbox(_L("Remember printer configuration"), _L("If enabled, Verslicer will remember and switch filament/process configuration for each printer automatically."), "remember_printer_config");
    ...
    auto item_bambu_smart_print = create_item_checkbox(
        _L("Smart Print assistant"),
        _L("Analyze plates, learn from Bambu Lab print failures, and suggest settings. "
           "Open the Smart Print tab in the toolbar or Preferences → Smart Print for full controls."),
        "bambu_smart_print_enabled");
    g_sizer->Add(item_bambu_smart_print);
    ...
    auto item_ai_coach = create_item_checkbox(
        ...
        _L("Show non-intrusive tips in the bottom-right corner (recommended off in Expert mode)."),
        "ai_coach_enabled");
    g_sizer->Add(item_ai_coach);

    auto item_ai_coach_bed = create_item_checkbox(
        _L("AI Coach: bed layout hints"),
        _L("Suggest auto-arrange when the build plate has a lot of free space (Advanced mode)."),
        "ai_coach_show_bed_hints");
    g_sizer->Add(item_ai_coach_bed);
    ...
    auto item_auto_reslice = create_item_auto_reslice(
        ...
        wxString::Format(_L("If enabled, %s will re-slice automatically whenever slicing-related settings change."),
                          SLIC3R_APP_FULL_NAME),
    ...
    auto item_stealth_mode     = create_item_checkbox(_L("Stealth mode"), _L("This disables all cloud services e.g. Cloud and Bambu Cloud. This stops the transmission of data to Bambu's cloud services too. Users who don't use BBL machines or use LAN mode only can safely turn on this function."), "stealth_mode");
...

    {
        ...
        text->SetToolTip(_L("Allow logging into Bambu Cloud alongside Cloud. When enabled, a Bambu login section appears on the homepage."));
    ...
    g_sizer->Add(create_item_title(wxString::Format(_L("Associate files to %s"), SLIC3R_APP_FULL_NAME)), 1, wxEXPAND);

    auto item_associate_3mf    = create_item_checkbox(
        wxString::Format(_L("Associate 3MF files to %s"), SLIC3R_APP_FULL_NAME),
        wxString::Format(_L("If enabled, sets %s as default application to open 3MF files."), SLIC3R_APP_FULL_NAME),
        "associate_3mf");
    g_sizer->Add(item_associate_3mf);

    auto item_associate_drc = create_item_checkbox(
        wxString::Format(_L("Associate DRC files to %s"), SLIC3R_APP_FULL_NAME),
        wxString::Format(_L("If enabled, sets %s as default application to open DRC files."), SLIC3R_APP_FULL_NAME),
        "associate_drc");
    g_sizer->Add(item_associate_drc);

    auto item_associate_stl    = create_item_checkbox(
        wxString::Format(_L("Associate STL files to %s"), SLIC3R_APP_FULL_NAME),
        wxString::Format(_L("If enabled, sets %s as default application to open STL files."), SLIC3R_APP_FULL_NAME),
        "associate_stl");
    g_sizer->Add(item_associate_stl);

    auto item_associate_step   = create_item_checkbox(
        wxString::Format(_L("Associate STEP files to %s"), SLIC3R_APP_FULL_NAME),
        wxString::Format(_L("If enabled, sets %s as default application to open STEP files."), SLIC3R_APP_FULL_NAME),
        "associate_step");
    g_sizer->Add(item_associate_step);

    //// ASSOCIATE > WebLinks
    g_sizer->Add(create_item_title(wxString::Format(_L("Associate web links to %s"), SLIC3R_APP_FULL_NAME)), 1, wxEXPAND);
    ...
    //// SMART PRINT TAB
    /////////////////////////////////////
    m_pref_tabs->AppendItem(_L("Smart Print"));
    f_sizers.push_back(new wxFlexGridSizer(1, 1, v_gap, 0));
    g_sizer = f_sizers.back();
    g_sizer->AddGrowableCol(0, 1);
    {
        auto* smart_panel = new BambuSmartPrintPanel(m_parent, BambuSmartPrintPanel::LayoutStyle::Preferences);
        g_sizer->Add(smart_panel, 0, wxEXPAND);
    }
    g_sizer->AddSpacer(FromDIP(10));
    sizer_page->Add(g_sizer, 0, wxEXPAND);
