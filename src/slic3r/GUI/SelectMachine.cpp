    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_ams_backup_tip->SetForegroundColour(wxColour("#FF8F4A"));
    ...
    m_link_edit_nozzle->SetForegroundColour("#FF8F4A");
    ...
    m_options_other->SetBackgroundColour(StateColor::darkModeColorFor(*wxWHITE));
    ...
    m_sizer_options = new wxFlexGridSizer(2, FromDIP(10), FromDIP(8));
    ...
    m_statictext_finish->SetForegroundColour(wxColour(255, 143, 74));
    ...
    m_scroll_sizer->Add(sizer_split_filament, 0, wxEXPAND|wxLEFT|wxRIGHT, FromDIP(15));
    ...
    m_scroll_sizer->Add(sizer_split_options, 0, wxEXPAND|wxLEFT|wxRIGHT, FromDIP(15));
    m_scroll_sizer->Add(0, 0, 0, wxTOP, FromDIP(10));
...
void SelectMachineDialog::sending_mode()
{
    ...
    const wxSize sending_book_size(FromDIP(645), FromDIP(64));
    m_simplebook->SetMinSize(sending_book_size);
    m_simplebook->SetMaxSize(sending_book_size);
...
void SelectMachineDialog::set_auto_send_on_ready(bool enable)
{
    m_auto_send_on_ready   = enable;
    m_auto_send_triggered = false;
}
...
{
    auto apply = [this, code, msg = std::move(msg), extra = std::move(extra)]() mutable {
        m_print_error_code  = code;
        m_print_error_msg   = std::move(msg);
        m_print_error_extra = std::move(extra);
        if (m_sw_print_failed_info && m_sw_print_failed_info->IsShown())
            show_print_failed_info(true);
    };
    if (wxIsMainThread())
        apply();
    else
        CallAfter(std::move(apply));
}
...
void SelectMachineDialog::show_status(PrintDialogStatus status, std::vector<wxString> params, wxString wiki_url)
{
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        Enable_Refresh_Button(true);
        Enable_Send_Button(true);
        if (m_auto_send_on_ready && !m_auto_send_triggered) {
            m_auto_send_triggered = true;
            CallAfter([this]() {
...
void SelectMachineDialog::on_ok_btn(wxCommandEvent &event)
{
    ...
    {
        if (m_auto_send_on_ready && !is_printing_block) {
            on_send_print();
            return;
        }
...
void SelectMachineDialog::update_options_layout()
{
    ...
    m_sizer_options->Clear(false);

    std::vector<PrintOption *> visible;
    visible.reserve(m_checkbox_list_order.size());
    for (auto option : m_checkbox_list_order) {
        if (option->IsShown())
            visible.push_back(option);
    }

    for (auto option : visible)
...
void SelectMachineDialog::on_send_print()
{
    ...
    if (!wxGetApp().getAgent()) {
        const std::string err = _u8L("Bambu network plugin is not loaded.");
        const std::string hint = _u8L("Enable it under Preferences → Online, then sign in and try again.");
        if (m_plater)
            m_plater->update_print_error_info(BAMBU_NETWORK_ERR_INVALID_HANDLE, err, hint);
        else if (wxGetApp().plater())
            wxGetApp().plater()->update_print_error_info(BAMBU_NETWORK_ERR_INVALID_HANDLE, err, hint);
        m_status_bar->show_error_info(_L("Failed to send the print job. Please try again."),
            BAMBU_NETWORK_ERR_INVALID_HANDLE, from_u8(err), from_u8(hint));
        show_print_failed_info(true, BAMBU_NETWORK_ERR_INVALID_HANDLE, from_u8(err), from_u8(hint));
        return;
    }
...
void SelectMachineDialog::update_user_printer()
{
    ...
    else
    {
        ...
        m_printer_box->GetPrinterComboBox()->SetToolTip(
            _L("No printer found yet:\n"
               "• Install the network plug-in (Preferences → Online)\n"
               "• Sign in to your Bambu Lab account\n"
               "• Power on the printer and use the same network"));
    }
...
void SelectMachineDialog::on_selection_changed(wxCommandEvent &event)
{
    ...
    const bool need_select_machine = !dev->get_selected_machine()
        || dev->get_selected_machine()->get_dev_id() != m_printer_last_select;
    const bool info_ready = obj->is_info_ready(!obj->is_lan_mode_printer());

    if (!obj->get_lan_mode_connection_state()) {
        obj->command_get_version();
        obj->command_request_push_all();
        if (need_select_machine) {
            dev->set_selected_machine(m_printer_last_select);
        }
        m_check_flag = false;
    } else if (!info_ready) {
        // LAN already selected in Device tab but push/version not complete yet
        obj->command_get_version();
...
void SelectMachineDialog::update_show_status(MachineObject* obj_)
{
    ...
    if (obj_->is_lan_mode_printer()) {
        if (obj_->get_dev_ip().empty()) {
            show_status(PrintDialogStatus::PrintStatusInvalidPrinter,
                {_L("Printer IP is not set. Add or bind the printer in the Device tab (LAN mode).")});
            return;
        }
        if (!obj_->has_access_right()) {
            show_status(PrintDialogStatus::PrintStatusInvalidPrinter,
                {_L("LAN access code is required. Enter the access code shown on the printer screen.")});
            return;
        }
    }

    if (!obj_->is_info_ready(!obj_->is_lan_mode_printer())) {
        if (is_timeout()) {
            m_ams_mapping_result.clear();
            sync_ams_mapping_result(m_ams_mapping_result);
            if (obj_->is_lan_mode_printer()) {
                show_status(PrintDialogStatus::PrintStatusReadingTimeout,
                    {_L("Synchronizing device information timed out. Check printer IP and LAN access code, then press refresh.")});
            } else {
                show_status(PrintDialogStatus::PrintStatusReadingTimeout);
            }
        } else {
            if (obj_->is_lan_mode_printer() && m_timeout_count > 0 && (m_timeout_count % 25) == 0) {
...
void SelectMachineDialog::UpdateStatusCheckWarning_ExtensionTool(MachineObject* obj_)
{
     ...
     : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
 {
     ...
     const int row_h = FromDIP(22) + FromDIP(8);
     SetMinSize(wxSize(-1, row_h));
     SetMaxSize(wxSize(-1, row_h));
...
void PrintOption::update_title_display()
{
    ...
    wxClientDC dc(m_printoption_title);
    dc.SetFont(m_printoption_title->GetFont());

    const int toggle_reserve = m_printoption_item ? m_printoption_item->GetMinSize().GetWidth() + FromDIP(36) : FromDIP(120);
    int maxTitleWidth = GetSize().GetWidth() - toggle_reserve;
    if (maxTitleWidth < FromDIP(100))
        maxTitleWidth = FromDIP(220);

    wxString displayTitle = m_full_title;
    if (dc.GetTextExtent(m_full_title).x > maxTitleWidth) {
...
{
    auto size = GetSize();
    const wxColour track_bg = StateColor::darkModeColorFor(wxColour(0xEE, 0xEE, 0xEE));
    dc.SetPen(wxPen(*wxTRANSPARENT_PEN));
    dc.SetBrush(track_bg);
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#FF8F4A") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
