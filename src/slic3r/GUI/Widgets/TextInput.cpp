                 ...
                 std::make_pair(0x262E30, (int) StateColor::Normal))
{
    radius = 0;
    border_width = 1;
    border_color = StateColor(std::make_pair(0xDBDBDB, (int) StateColor::Disabled), std::make_pair(0xFF8F4A, (int) StateColor::Hovered),
...
void TextInput::render(wxDC& dc)
{
    ...
    if (!text.IsEmpty()) {
        if (static_tips.IsEmpty()) {
            wxSize textSize = text_ctrl ? text_ctrl->GetSize() : wxSize{};
            // Read-only ComboBox hides the inner TextCtrl; don't offset the label by its width.
            const bool label_only = !text_ctrl || !text_ctrl->IsShown();
            if (align_right || align_center || label_only)
            {
