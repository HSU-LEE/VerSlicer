...
DropDown::DropDown(std::vector<Item> &items)
    ...
    , selector_border_color(std::make_pair(0xFF8F4A, (int) StateColor::Hovered),
        std::make_pair(wxColour(0x4A, 0x4A, 0x4A), (int) StateColor::Normal))
    // In dark UI we don't want the dropdown selection to flash white.
    // Use neutral greys for selected/normal backgrounds; hovered state still uses selector_border_color.
    , selector_background_color(std::make_pair(wxColour(0x3A, 0x3F, 0x43), (int) StateColor::Checked),
        std::make_pair(wxColour(0x2B, 0x2F, 0x33), (int) StateColor::Normal))
{
}
