...
SwitchButton::SwitchButton(wxWindow* parent, wxWindowID id)
    ...
    , thumb_color(std::pair{0xFF8F4A, (int) StateColor::Checked}, std::pair{0xD9D9D9, (int) StateColor::Normal})
{
...
MultiSwitchButton::MultiSwitchButton(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size, long style)
    ...
    , m_bg_color(StateColor(
          ...
          std::make_pair(0xFF8F4A, (int) StateColor::Normal)))
...
void SwitchBoard::doRender(wxDC &dc)
{
	...
	/*left*/
    if (switch_left) {
        is_enable ? dc.SetBrush(wxBrush(wxColour(255, 143, 74))) : dc.SetBrush(disable_color);
	...
	/*right*/
    if (switch_right) {
        if (is_enable) {dc.SetBrush(wxBrush(wxColour(255, 143, 74)));
        } else {dc.SetBrush(disable_color);}
