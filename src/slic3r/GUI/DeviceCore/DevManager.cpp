            ...
            if (selected_machine == dev_id)
            {
                ...
                else
                {
                    ...
                    if (m_agent)
                    {
                        ...
                        const int conn_ret = it->second->connect(Slic3r::GUI::wxGetApp().app_config->get("enable_ssl_for_mqtt") == "true" ? true : false);
#else
                        const int conn_ret = it->second->connect(it->second->local_use_ssl);
#endif
                        it->second->set_lan_mode_connection_state(conn_ret == 0);
                    }
                }
            }
            else
