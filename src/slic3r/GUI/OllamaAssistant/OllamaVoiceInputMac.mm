...
static void deliver_voice_final(OllamaVoiceInput::FinalTextCallback cb, std::string text)
{
    if (!cb)
        return;
    wxGetApp().CallAfter([cb = std::move(cb), text = std::move(text)]() {
        if (cb)
            cb(text);
    });
}

static void deliver_voice_error(OllamaVoiceInput::ErrorCallback cb, std::string message)
{
    if (!cb)
        return;
    wxGetApp().CallAfter([cb = std::move(cb), message = std::move(message)]() {
        if (cb)
            cb(message);
    });
}
...
struct VoiceSession
{
    ...
    void teardown(bool cancel_task)
    {
        ...
        deliver_delay_pending.store(false);
                                        ...
                                        const std::string& text)
{
    ...
    deliver_voice_final(std::move(on_final), text);
    return true;
}

static void schedule_deliver_recognized_text(const std::shared_ptr<MacVoiceShared>& sh,
                                             const std::shared_ptr<VoiceSession>& session,
                                             bool finish_session_if_undelivered = false)
{
    if (!session || session->result_delivered.load(std::memory_order_acquire))
        return;

    bool expected = false;
    if (!session->deliver_delay_pending.compare_exchange_strong(expected, true))
        return;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (s)
            s->deliver_delay_pending.store(false);
        if (!is_current_session(sh2, s) || s->result_delivered.load(std::memory_order_acquire))
            return;
        if (!try_deliver_recognized_text(sh2, s, s->last_partial_text) && finish_session_if_undelivered)
            finish_session(sh2, s, /*cancel_task*/ false);
    });
}
...
static void fail_start(const std::shared_ptr<MacVoiceShared>& sh, const std::string& message)
{
    if (!sh)
        return;
    deliver_voice_error(sh->on_error, message);
    sh->listening.store(false);
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (error) {
                    if (is_speech_cancel_error(error) || is_no_speech_detected_error(error)) {
                        if (finishing) {
                            schedule_deliver_recognized_text(sh2, s2, /*finish_session_if_undelivered*/ true);
                        } else if (is_no_speech_detected_error(error)) {
                            deliver_voice_error(s2->on_error,
                                "No speech detected. Speak while listening, then tap the mic again to finish.");
                        }
                        return;
                    }
                    const std::string msg = "Speech recognition error: " + nserr_to_string(error);
                    deliver_voice_error(s2->on_error, msg);
                ...
                if (result.isFinal || finishing) {
                    schedule_deliver_recognized_text(
                        sh2, s2, /*finish_session_if_undelivered*/ finishing && result.isFinal);
                }
            });
        }];
