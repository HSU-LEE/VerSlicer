                                                          ...
                                                          bool include_makerworld)
{
    ...
    const bool wants_brim       = contains_brim_intent(user_req) || contains_adhesion_intent(user_req);
    const bool wants_durability = contains_durability_intent(user_req);
    const bool wants_strength   = (contains_strength_intent(user_req) || wants_durability)
        && !contains_explicit_infill_intent(user_req);
    const bool wants_no_brim  = contains_disable_brim_intent(user_req);
    const bool wants_flip     = contains_flip_intent(user_req);
    const bool wants_rotate   = contains_rotate_intent(user_req);
    const bool wants_place    = contains_placement_intent(user_req) && !describes_print_quality_symptom(user_req);
    const bool wants_file     = contains_file_intent(user_req);
    const bool vague_fix      = OllamaIntentContext::user_vague_fix_request(user_req);
    const bool wants_warp     = OllamaIntentContext::user_wants_warp_relief(user_req);
    const bool slice_support  = OllamaIntentContext::slice_suggests_support();
    const bool wants_stringing = OllamaIntentContext::user_wants_stringing_relief(user_req);
    const bool wants_top       = OllamaIntentContext::user_wants_top_surface_quality(user_req);
    const bool wants_first_layer = OllamaIntentContext::user_wants_first_layer_help(user_req);
    const bool wants_lay_flat  = OllamaIntentContext::user_wants_lay_flat(user_req);
    const OllamaSelectionFootprint footprint = OllamaIntentContext::current_selection_footprint();
    const bool tall_narrow = footprint.valid
        && ollama_selection_is_tall_narrow(footprint.x_mm, footprint.y_mm, footprint.z_mm);
    const double recommended_brim_w = ollama_recommended_brim_width_mm(footprint.x_mm, footprint.y_mm);
    ...
    if (wants_rotate && !scan.has_rotate) {
        ...
        scan.has_rotate = true;
    } else if (tall_narrow && (wants_lay_flat || vague_fix) && !scan.has_rotate) {
