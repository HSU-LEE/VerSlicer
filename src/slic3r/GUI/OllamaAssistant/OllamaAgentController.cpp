...
std::string build_agent_system_prompt(bool korean)
{
    const std::string tools = OllamaToolRegistry::agent_tools_schema_block(korean);
    if (korean) {
        return std::string(
                   "당신은 VerSlicer 내부 에이전트입니다. 사용자 목표를 달성할 때까지 도구를 연속 호출하세요.\n"
    ...
    return std::string(
               "You are the VerSlicer in-app agent. Call tools in a loop until the user's goal is met.\n"
                                     ...
                                     OllamaAgentCallbacks callbacks, int max_steps)
{
    ...
    m_awaiting_slice = false;
    m_step           = 0;
    m_max_steps      = max_steps > 0 ? max_steps : kDefaultMaxSteps;
    m_policy         = policy;
    m_parent         = parent;
    ...
    m_messages.push_back({"system", build_agent_system_prompt(ko)});

    const std::string state = OllamaAgentStateService::snapshot_json();
    std::string       user_block = std::string("Goal:\n") + user_goal + "\n\nInitial state (JSON):\n" + state;
    const nlohmann::json plan_hint = OllamaAgentGoalPlanner::build_plan_hint(user_goal, ko);
    if (plan_hint.value("multi_step", false))
        user_block += std::string("\n\nSuggested plan (JSON):\n") + plan_hint.dump(2);
    m_messages.push_back({"user", std::move(user_block)});
...
void OllamaAgentController::begin_step()
{
    ...
    if (m_callbacks.on_thinking) {
        const bool ko = wxGetApp().current_language_code().StartsWith("ko");
        m_callbacks.on_thinking(ko ? wxString::Format(wxString::FromUTF8("에이전트 단계 %d…"), m_step)
                                   : wxString::Format("Agent step %d…", m_step));
    }
...
void OllamaAgentController::on_llm_response(const std::string& text, const std::string& error)
{
    ...
    AIGuiOrchestrator::instance().on_chat_apply_end(applied, root, "ollama_agent");

    // Verifier pass after set_config
    if (root.contains("actions") && root["actions"].is_array()) {
        for (const auto& a : root["actions"]) {
            ...
            if (!mismatches.empty()) {
                nlohmann::json verify_note = nlohmann::json::object({
                    {"tool", "verify_config"},
                    {"ok", false},
                    {"changed", false},
                ...
                m_step_tool_results.back().push_back(verify_note);
            }
        }
    }
...
bool OllamaAgentController::execute_agent_root(const nlohmann::json& root)
{
    ...
    AIGuiOrchestrator::instance().on_chat_apply_end(applied, work, "ollama_agent");
