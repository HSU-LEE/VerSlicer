...
static SmartPrintWorkflowContent workflow_content_preferred(const PreparedPlateWorkflow& prep,
                                                            const BambuSmartPrint::ReadinessReport& readiness)
{
    if (BambuSmartPrint::PrintGoalSession::instance().has_last_plan())
        return PrintPlannerGui::workflow_content_from_plan(
            BambuSmartPrint::PrintGoalSession::instance().last_plan());
    return workflow_content_from(prep, readiness);
}
...
void BambuSmartPrintService::sync_from_plan(const BambuSmartPrint::PrintPlan& plan)
{
    m_last_mesh_analysis = plan.mesh;
    m_last_baseline      = plan.base_config;
    m_last_applied       = plan.proposed_config;
...
void BambuSmartPrintService::run_one_click_print(Plater* plater)
{
    ...
    try {
        ...
        if (m_last_readiness.tier == BambuSmartPrint::ReadinessTier::Risky) {
            ...
            m_silent_workflow = false;
            SmartPrintWorkflowContent content = workflow_content_preferred(prep, m_last_readiness);
...
void BambuSmartPrintService::run_auto_workflow(Plater* plater)
{
    ...
    try {
        ...
        SmartPrintWorkflowContent content;
        content = workflow_content_preferred(prep, m_last_readiness);
        PartPlate* plate = plater->get_partplate_list().get_curr_plate();
...
void BambuSmartPrintService::flush_pending_failure_dialog()
{
    ...
    if (AICoachController::is_enabled_for_current_mode()) {
        std::string summary = pending.record.diagnosis.description;
        if (summary.empty())
            summary = pending.record.diagnosis.action_line;
        if (summary.empty())
            summary = _u8L("The last print did not finish successfully. I can suggest safer settings.");

        nlohmann::json root;
        root["message"] = summary;
        nlohmann::json actions = nlohmann::json::array();
        for (const auto& ch : pending.fixes.changes) {
            if (actions.size() >= 6)
                break;
            actions.push_back(nlohmann::json{
                {"type", "set_config"},
                {"preset", "print"},
                {"options", nlohmann::json::object({{ch.key, ch.new_value}})},
            });
        }
        root["actions"] = actions;
        AIGuiOrchestrator::instance().on_print_failure_hint(plater, summary, root);
    } else {
...
void BambuSmartPrintService::handle_print_success(MachineObject* obj)
{
    ...
    if (plater && AICoachController::is_enabled_for_current_mode()) {
        const std::string job_name = obj->subtask_name.empty() ? obj->m_gcode_file : obj->subtask_name;
        wxGetApp().CallAfter([plater, job_name]() {
            if (wxGetApp().plater())
                AIGuiOrchestrator::instance().on_print_success(plater, job_name);
        });
    }
}
                                                    ...
                                                    const std::string& new_state)
{
    ...
    if (is_running_print_state(new_state)) {
        if (Plater* plater = wxGetApp().plater()) {
            const int percent = obj->mc_print_percent;
            wxGetApp().CallAfter([plater, percent]() {
                if (wxGetApp().plater())
                    AIGuiOrchestrator::instance().on_print_running(plater, percent);
            });
        }
    }
