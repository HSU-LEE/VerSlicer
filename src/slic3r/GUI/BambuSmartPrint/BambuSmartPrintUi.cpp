...
void style_orca_combobox(ComboBox* combo)
{
    ...
    const wxColour bg     = Theme::surface();
    const wxColour border = Theme::border();
    combo->SetBorderColor(StateColor(
        std::make_pair(border, (int) StateColor::Disabled),
        std::make_pair(Theme::primary(), (int) StateColor::Hovered),
        std::make_pair(border, (int) StateColor::Normal)));
    combo->SetBackgroundColor(StateColor(
        std::make_pair(Theme::surface_alt(), (int) StateColor::Disabled),
        std::make_pair(Theme::primary_soft(), (int) StateColor::Focused),
        std::make_pair(bg, (int) StateColor::Normal)));
    combo->SetLabelColor(StateColor(
        std::make_pair(Theme::text_muted(), (int) StateColor::Disabled),
        std::make_pair(Theme::text(), (int) StateColor::Normal)));
    ...
    drop.SetSelectorBackgroundColor(StateColor(
        std::make_pair(Theme::surface_alt(), (int) StateColor::Checked),
        std::make_pair(Theme::surface(), (int) StateColor::Normal)));
    drop.SetSelectorBorderColor(StateColor(
        std::make_pair(Theme::primary(), (int) StateColor::Hovered),
        std::make_pair(Theme::border(), (int) StateColor::Normal)));
}

void sync_combobox_selection(ComboBox* combo, int selection)
{
    if (!combo)
        return;
    ...
    if (count == 0) {
        ...
        combo->SetLabel(wxEmptyString);
        return;
    }
    selection = std::max(0, std::min(selection, (int) count - 1));
    const wxString label = combo->GetString((unsigned) selection);
    combo->SetSelection(selection);
    combo->SetValue(label);
    combo->SetTextLabel(label);
    combo->Refresh();
}
...
wxSizer* add_settings_choice_row(wxWindow* parent, wxSizer* target, const wxString& title,
                                 wxChoice** out, const std::vector<wxString>& choices,
                                 const wxString& tooltip)
{
    ...
    wxArrayString choice_items;
    for (const wxString& item : choices)
        choice_items.Add(item);
    auto* choice = new wxChoice(parent, wxID_ANY, wxDefaultPosition,
                                parent->FromDIP(wxSize(240, 28)), choice_items);
    choice->SetMinSize(parent->FromDIP(wxSize(240, 28)));
    choice->SetFont(Label::Body_14);
    choice->SetBackgroundColour(Theme::surface());
    ...
    row->Add(choice, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, parent->FromDIP(8));
                                   ...
                                   const wxString& tooltip)
{
    ...
    if (!choices.empty())
        sync_combobox_selection(combo, 0);
