    ...
    , m_style(style)
{
    ...
    add_settings_choice_row(assistant_body, assistant_inner, _L("When a model is loaded"),
        &m_auto_load_choice,
        {
    ...
    if (m_auto_load_choice) {
        const int mode = std::max(0, std::min(2, int(BambuSmartPrintService::auto_load_mode())));
        m_auto_load_choice->SetSelection(mode);
        m_auto_load_choice->Bind(wxEVT_CHOICE, [this](wxCommandEvent& e) {
