...
namespace {
static bool print_job_can_use_lan(const PrintJob* job, const PrintParams& params)
{
    return job && job->has_sdcard && !params.dev_ip.empty() && !params.password.empty();
}

static int print_job_connect_lan(MachineObject* obj)
{
    if (!obj || obj->get_dev_ip().empty())
        return -1;
#if !BBL_RELEASE_TO_PUBLIC
    const bool use_ssl = wxGetApp().app_config && wxGetApp().app_config->get("enable_ssl_for_mqtt") == "true";
#else
    const bool use_ssl = obj->local_use_ssl;
#endif
    return obj->connect(use_ssl);
}

static int print_job_send_over_lan(
    NetworkAgent* agent,
    PrintParams& params,
    bool could_emmc_print,
    bool disable_emmc,
    const OnUpdateStatusFn& update_fn,
    const WasCancelledFn& cancel_fn,
    const OnWaitFn& wait_fn)
{
    params.connection_type = "lan";
    if (could_emmc_print && !disable_emmc)
        return agent->start_local_print(params, update_fn, cancel_fn);
    return agent->start_local_print_with_record(params, update_fn, cancel_fn, wait_fn);
}
} // namespace
...
static auto cloud_signed_error_str = _u8L(
    "Cloud print is blocked for this slicer (error -26). Enable LAN mode and Developer mode on the printer, "
    "then set the printer IP and LAN access code in the Device tab and print again.");
static auto lan_then_cloud_failed_str = _u8L(
    "LAN and cloud upload both failed (error -26). Check printer IP, LAN access code, and Developer mode.");
...
void PrintJob::process(Ctl &ctl)
{
    ...
    if (!m_agent) {
        wxGetApp().ensure_bambu_network_plugin_installed(true);
        m_agent = wxGetApp().getAgent();
    }
    if (!m_agent) {
        const std::string err = _u8L("Bambu network plug-in is not loaded (error -1). Install it under Preferences → Online, then restart.");
        ctl.update_status(-1, err);
        m_plater->update_print_error_info(BAMBU_NETWORK_ERR_INVALID_HANDLE, err, "");
        ctl.show_error_info(err, BAMBU_NETWORK_ERR_INVALID_HANDLE, err, "");
        return;
    }
    ...
    if (m_print_type == "from_sdcard_view") {
        ...
        if (!wxGetApp().app_config->get("lan_mode_only").empty() && wxGetApp().app_config->get("lan_mode_only") == "1") {
            ...
            // Third-party slicers are often rejected by Bambu cloud (error -26). Prefer LAN when possible.
            if (print_job_can_use_lan(this, params)) {
                BOOST_LOG_TRIVIAL(info) << "print_job: try LAN first (ip=" << params.dev_ip << ")";
                print_job_connect_lan(obj);
                ctl.update_status(curr_percent, _u8L("Sending print job over LAN"));
                is_try_lan_mode = true;
                result = print_job_send_over_lan(m_agent, params, this->could_emmc_print, disable_emmc, update_fn, cancel_fn, wait_fn);
                if (result == 0) {
                    params.comments = "";
                ...
                if (result < 0) {
                    ...
                    BOOST_LOG_TRIVIAL(warning) << "print_job: LAN failed (" << result << "), trying cloud";
                    params.connection_type = this->connection_type;
                    ctl.update_status(curr_percent, _u8L("Sending print job through cloud service"));
                    result = m_agent->start_print(params, update_fn, cancel_fn, wait_fn);
                }
            } else {
                BOOST_LOG_TRIVIAL(info) << "print_job: send with cloud (no LAN credentials)";
                ctl.update_status(curr_percent, _u8L("Sending print job through cloud service"));
                result = m_agent->start_print(params, update_fn, cancel_fn, wait_fn);
            }
        }
    ...
    if (result == BAMBU_NETWORK_SIGNED_ERROR && print_job_can_use_lan(this, params)) {
        BOOST_LOG_TRIVIAL(warning) << "print_job: cloud signed error -26, retrying via LAN";
        print_job_connect_lan(obj);
        ctl.update_status(30, _u8L("Cloud blocked; sending print job over LAN"));
        is_try_lan_mode = true;
        result = print_job_send_over_lan(m_agent, params, this->could_emmc_print, disable_emmc, update_fn, cancel_fn, wait_fn);
        if (result == 0)
            params.comments = "lan_after_signed_error";
    }
    ...
    if (result < 0) {
        ...
        if (result == BAMBU_NETWORK_ERR_PRINT_WR_FILE_NOT_EXIST || result == BAMBU_NETWORK_ERR_PRINT_SP_FILE_NOT_EXIST) {
            ...
            msg_text = failed_in_cloud_service_str;
                   ...
                   || result == BAMBU_NETWORK_ERR_PRINT_WR_GET_NOTIFICATION_TIMEOUT
            ...
            msg_text = cloud_signed_error_str;
            if (m_enter_ip_address_fun_fail && (params.dev_ip.empty() || params.password.empty())) {
                ctl.call_on_main_thread([this]() { m_enter_ip_address_fun_fail(); }).wait();
            }
        } else {
            msg_text = send_print_failed_str;
        }

        std::string err_extra = params.comments;
        if (err_extra.empty() && !error_text.empty())
            err_extra = error_text.ToUTF8().data();
        if (err_extra.empty())
            err_extra = (boost::format("connection=%1%") % connection_type).str();
        if (result == BAMBU_NETWORK_SIGNED_ERROR
            && (err_extra.find("failed") != std::string::npos
                || err_extra.find("lan_direct") != std::string::npos))
            err_extra = lan_then_cloud_failed_str;

        m_plater->update_print_error_info(result, msg_text, err_extra);

        if (result != BAMBU_NETWORK_ERR_CANCELED)
            ctl.show_error_info(msg_text, result, msg_text, err_extra);

        BOOST_LOG_TRIVIAL(error) << "print_job: failed, result = " << result << " extra=" << err_extra;
    } else {
