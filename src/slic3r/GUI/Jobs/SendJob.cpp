...
void SendJob::process(Ctl &ctl)
{
    ...
    if (params.connection_type != "lan") {
        ...
        if (!params.password.empty()
            ...
            if (result < 0) {
                BOOST_LOG_TRIVIAL(warning) << "send_job: LAN FTP failed (" << result << "), retry via cloud";
                ctl.update_status(curr_percent, sending_over_cloud_str);
                PrintParams cloud_params = params;
                cloud_params.dev_ip.clear();
                cloud_params.password.clear();
                if (params.comments.empty())
                    cloud_params.comments = "lan_fallback";
                result = agent->start_send_gcode_to_sdcard(cloud_params, update_fn, cancel_fn, nullptr);
            }
        } else {
            BOOST_LOG_TRIVIAL(info) << "send_job: send gcode via cloud (no LAN credentials)";
            ctl.update_status(curr_percent, sending_over_cloud_str);
            result = agent->start_send_gcode_to_sdcard(params, update_fn, cancel_fn, nullptr);
        }
    } else {
    ...
    if (result < 0) {
        ...
        BOOST_LOG_TRIVIAL(error) << "send_job: failed, result = " << result << " extra=" << err_extra;
