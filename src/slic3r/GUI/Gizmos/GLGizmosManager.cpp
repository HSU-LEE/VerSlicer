...
bool GLGizmosManager::init_icon_textures()
{
    ...
    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_chat.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_CHAT, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_chat_hover.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_CHAT_HOVER, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_chat_dark.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_CHAT_DARK, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_chat_dark_hover.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_CHAT_DARK_HOVER, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_mic.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MIC, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_mic_hover.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MIC_HOVER, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_mic_dark.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MIC_DARK, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/web/image/canvas_mic_dark_hover.svg", 72, 72, texture_id))
        icon_list.insert(std::make_pair((int) IC_CANVAS_MIC_DARK_HOVER, texture_id));
    else
        return false;
