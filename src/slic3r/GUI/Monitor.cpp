    ...
    m_select_machine(SelectMachinePopup(this))
{
    ...
    m_smart_print_summary = new BambuSmartPrintDeviceSummary(this);

    m_main_sizer = new wxBoxSizer(wxVERTICAL);
    m_main_sizer->Add(m_smart_print_summary, 0, wxEXPAND);
...
void MonitorPanel::update_all()
{
    ...
    if (!obj) {
        if (m_smart_print_summary)
            m_smart_print_summary->update_for_printer("");
    ...
    if (m_smart_print_summary)
        m_smart_print_summary->update_for_printer(obj->get_dev_id());
