...
void ButtonsListCtrl::SetSelection(int sel)
{
    ...
    wxColour selected_btn_bg("#FF8F4A");    // Gradient #FF8F4A
    if (m_selection >= 0) {
    ...
    StateColor bg_color = StateColor(
        std::pair{wxColour(255, 143, 74), (int) StateColor::Hovered},
        std::pair{wxColour(255,143, 74), (int) StateColor::Normal});
    m_pageButtons[m_selection]->SetBackgroundColor(bg_color);
...
void ButtonsListCtrl::SetPageVisible(size_t n, bool visible)
{
    if (n >= m_pageButtons.size())
        return;
    m_pageButtons[n]->Show(visible);
    m_sizer->Layout();
    Refresh();
}
