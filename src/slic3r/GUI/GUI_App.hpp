...
enum VerslicerMenuIDs {
  VerslicerMenuAbout,
  VerslicerMenuPreferences,
};
...
public:
    ...
    std::string logo_name() const { return is_editor() ? "Verslicer" : "Verslicer-gcodeviewer"; }
...
//#endif
    ...
    OllamaChatDialog* ollama_chat_dialog{ nullptr };
    std::unique_ptr<OllamaVoiceInput> ollama_voice_input;
    ...
    // Download/load Bambu network plugin if missing (SlicePilot / print & device control).
    bool            ensure_bambu_network_plugin_installed(bool offer_download = true);
    void            notify_bambu_network_plugin_required();
