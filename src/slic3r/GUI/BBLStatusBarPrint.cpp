    ...
    , m_sizer(new wxBoxSizer(wxHORIZONTAL))
{
    ...
    m_link_show_error->SetForegroundColour(wxColour("#FF8F4A"));
    ...
    m_prog->SetForegroundColour(wxColour(255, 143, 74));
    m_prog->SetValue(0);
...
void BBLStatusBarPrint::show_error_info(wxString msg, int code, wxString description, wxString extra)
{
    ...
    const int panel_w = m_self->FromDIP(550);
    const int text_h  = m_self->FromDIP(52);
    top_panel->SetMinSize(wxSize(panel_w, text_h));
    top_panel->SetMaxSize(wxSize(panel_w, text_h));
    m_status_text->SetMaxSize(wxSize(m_self->FromDIP(480), text_h));
    m_status_text->SetMinSize(wxSize(m_self->FromDIP(480), text_h));
    m_status_text->Wrap(m_self->FromDIP(470));
