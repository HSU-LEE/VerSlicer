...
void SendToPrinterDialog::on_ok(wxCommandEvent &event)
{
   ...
   /* std::string  file_name       = "";
    ...
    else
    {
        ...
        m_send_job->cloud_print_only    = obj_->is_support_cloud_print_only;
...
void SendToPrinterDialog::on_selection_changed(wxCommandEvent &event)
{
    ...
    if (!obj->get_lan_mode_connection_state()) {
        ...
        if (need_select_machine) {
            if (dev->get_selected_machine() && dev->get_selected_machine()->get_dev_id() != m_printer_last_select) {
...
void SendToPrinterDialog::update_show_status()
{
    ...
    if (!obj_->is_info_ready(!obj_->is_lan_mode_printer())) {
        ...
        if (obj_->is_lan_mode_printer() && timeout_count > 0 && (timeout_count % 25) == 0) {
...
void SendToPrinterDialog::show_status(PrintDialogStatus status, std::vector<wxString> params)
{
	...
	if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        wxString msg_text = _L("The printer is required to be in the same LAN as Verslicer.");
        update_print_status_msg(msg_text, true, true);
        Enable_Send_Button(false);
        Enable_Refresh_Button(true);
    }
