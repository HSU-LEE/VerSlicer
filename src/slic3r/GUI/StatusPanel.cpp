...
/* colors */
static wxColour STATUS_PANEL_BG()     { return StateColor::darkModeColorFor(wxColour("#EEEEEE")); }
static wxColour STATUS_TITLE_BG()     { return StateColor::darkModeColorFor(wxColour("#F8F8F8")); }
static wxColour STATIC_BOX_LINE_COL() { return StateColor::darkModeColorFor(wxColour("#EEEEEE")); }

static wxColour BUTTON_NORMAL1_COL() { return StateColor::darkModeColorFor(wxColour("#EEEEEE")); }
static wxColour BUTTON_NORMAL2_COL() { return StateColor::darkModeColorFor(wxColour("#CECECE")); }
static wxColour BUTTON_PRESS_COL()   { return StateColor::darkModeColorFor(wxColour("#ACACAC")); }
static wxColour BUTTON_HOVER_COL()   { return StateColor::darkModeColorFor(wxColour("#FF8F4A")); }

static wxColour DISCONNECT_TEXT_COL() { return StateColor::darkModeColorFor(wxColour("#ACACAC")); }
static wxColour NORMAL_TEXT_COL()     { return StateColor::darkModeColorFor(wxColour("#303A3C")); }
static wxColour NORMAL_FAN_TEXT_COL() { return StateColor::darkModeColorFor(wxColour("#6B6B6B")); }
static wxColour WARNING_INFO_BG_COL() { return StateColor::darkModeColorFor(wxColour("#FF6F00")); }
static wxColour STAGE_TEXT_COL()      { return StateColor::darkModeColorFor(wxColour("#FF8F4A")); }

static wxColour GROUP_STATIC_LINE_COL() { return StateColor::darkModeColorFor(wxColour("#CECECE")); }
...
static wxColour PAGE_TITLE_FONT_COL()  { return StateColor::darkModeColorFor(wxColour("#6B6B6B")); }
static wxColour GROUP_TITLE_FONT_COL() { return StateColor::darkModeColorFor(wxColour("#ACACAC")); }
static wxColour TEXT_LIGHT_FONT_COL()  { return StateColor::darkModeColorFor(wxColour("#6B6B6B")); }
...
void PrintingTaskPanel::create_panel(wxWindow* parent)
{
    ...
    m_staticText_progress_percent->SetForegroundColour(wxColour(255, 143, 74));
    ...
    m_staticText_progress_percent_icon->SetForegroundColour(wxColour(255, 143, 74));
...
wxBoxSizer *StatusBasePanel::create_monitoring_page()
{
    ...
    m_bmToggleBtn_timelapse->Bind(wxEVT_TOGGLEBUTTON, [this](wxCommandEvent &e) {
        ...
        if (!agent)
            return;
        if (e.IsChecked())
            agent->start_subscribe("tunnel");
        else
            agent->stop_subscribe("tunnel");
    });
#endif
...
wxBoxSizer *StatusBasePanel::create_temp_control(wxWindow *parent)
{
    ...
    StateColor tempinput_border_colour(std::make_pair(StateColor::darkModeColorFor(*wxWHITE), (int)StateColor::Disabled), std::make_pair(BUTTON_HOVER_COL(), (int)StateColor::Focused),
        std::make_pair(BUTTON_HOVER_COL(), (int)StateColor::Hovered), std::make_pair(StateColor::darkModeColorFor(*wxWHITE), (int)StateColor::Normal));
...
wxBoxSizer *StatusBasePanel::create_misc_control(wxWindow *parent)
{
    ...
    m_switch_fan->Bind(wxEVT_ENTER_WINDOW, [this](auto& e) {
        m_fan_panel->SetBackgroundColor(wxColour(255, 143, 74));
        });
...
void StatusBasePanel::reset_extruder_ui_disconnected()
{
    if (!m_extruder_book || m_extruderImage.empty())
        return;

    m_nozzle_num = 0;
    m_extruder_book->SetSelection(1);
    m_extruderImage[0]->setExtruderCount(1);
    m_extruderImage[0]->update(ExtruderState::EMPTY_LOAD);
    m_extruderImage[0]->Refresh();

    if (m_nozzle_btn_panel)
        m_nozzle_btn_panel->Hide();
    if (m_extruder_switching_status)
        m_extruder_switching_status->Hide();
    if (m_extruder_label)
        m_extruder_label->Show(true);
}
...
wxBoxSizer *StatusBasePanel::create_extruder_control(wxWindow *parent)
{
    ...
    {
        auto* placeholder = new wxPanel(panel);
        placeholder->SetBackgroundColour(panel->GetBackgroundColour());
        m_extruder_book->InsertPage(0, placeholder, "");
    }
...
wxBoxSizer* StatusBasePanel::create_filament_group(wxWindow* parent)
{
    ...
    m_title_filament_loading->SetForegroundColour(wxColour(255, 130, 70));
...
void StatusPanel::update(MachineObject *obj)
{
    if (!obj || !obj->is_info_ready())
    {
        ...
        reset_extruder_ui_disconnected();
        return;
    }
...
{
    const int fan_id = event.GetInt();
    const int speed  = atoi(event.GetString().c_str());
    if (obj && fan_id > 0) {
        if (!obj->is_enable_np)
            obj->GetFan()->command_control_fan(fan_id, speed);
        else
            obj->GetFan()->command_control_fan_new(fan_id, speed);
    }
