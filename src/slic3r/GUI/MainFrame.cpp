...
#ifdef __APPLE__
class VerslicerTaskBarIcon : public wxTaskBarIcon
{
public:
    VerslicerTaskBarIcon(wxTaskBarIconType iconType = wxTBI_DEFAULT_TYPE) : wxTaskBarIcon(iconType) {}
...
#else // _WIN32
    return wxIcon(Slic3r::var("Verslicer.ico"), wxBITMAP_TYPE_ICO);
#endif // _WIN32
}
...
*/
    ...
    case GUI_App::EAppMode::Editor:
        m_taskbar_icon = std::make_unique<VerslicerTaskBarIcon>(wxTBI_DOCK);
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("Verslicer-mac_256px.ico"), wxBITMAP_TYPE_ICO), SLIC3R_APP_FULL_NAME);
        break;
...
//        }, wxID_HIGHEST + wxID_PREFERENCES);
    ...
    wxGetApp().CallAfter([this]() {
        if (m_tabpanel)
            SlicePilotSimpleLayout::apply(this);
    });
...
void MainFrame::init_tabpanel() {
    ...
    m_tabpanel->Bind(wxEVT_NOTEBOOK_PAGE_CHANGED, [this](wxBookCtrlEvent& e) {
        ...
        if (panel == m_smart_print_page) {
            m_last_selected_tab = sel;
            if (m_smart_print_panel)
                m_smart_print_panel->refresh_all();
            if (panel)
                panel->SetFocus();
            return;
        }
    ...
    m_smart_print_page = new wxPanel(m_tabpanel, wxID_ANY);
    {
        SlicePilotUi::apply_panel_chrome(m_smart_print_page);
        auto* page_sz = new wxBoxSizer(wxVERTICAL);
        m_smart_print_panel = new BambuSmartPrintPanel(m_smart_print_page, BambuSmartPrintPanel::LayoutStyle::Full);
        page_sz->Add(m_smart_print_panel, 1, wxEXPAND);
        m_smart_print_page->SetSizer(page_sz);
    }
    // Match Device tab: same icon for active/inactive, standard label (InsertPage adds leading space).
    m_tabpanel->AddPage(m_smart_print_page, _L("Smart Print"),
                        std::string("tab_smart_print_active"), std::string("tab_smart_print_active"), false);
...
bool MainFrame::get_enable_print_status()
{
    ...
    else if (m_print_select == ePrintPlate)
    {
        ...
        if (GUI::BambuSmartPrintService::is_enabled()
            ...
            enable = enable && !m_plater->model().objects.empty();
...
void MainFrame::update_side_button_style()
{
    ...
    StateColor m_btn_bg_enable = StateColor(
        std::pair<wxColour, int>(wxColour(255, 130, 70), StateColor::Pressed),
        std::pair<wxColour, int>(wxColour(48, 221, 112), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(255, 143, 74), StateColor::Normal)
    );
...
// See https://github.com/Verslicer/Verslicer/issues/8152
...
static wxMenu* generate_help_menu()
{
    ...
    // Report a bug
    //append_menu_item(helpMenu, wxID_ANY, _L("Report Bug(TODO)"), _L("Report a bug of Verslicer"),
    //    [](wxCommandEvent&) {
    //        //TODO
    //    });
    // Check New Version
...
void MainFrame::init_menubar_as_editor()
{
    ...
    //auto preference_item = new wxMenuItem(parent_menu, VerslicerMenuPreferences + bambu_studio_id_base, _L("Preferences") + "\t" + ctrl + ",", "");
#else
...
//    });
...
#ifdef __APPLE__
    wxString about_title = wxString::Format(_L("&About %s"), SLIC3R_APP_FULL_NAME);
    //auto about_item = new wxMenuItem(parent_menu, VerslicerMenuAbout + bambu_studio_id_base, about_title, "");
        //parent_menu->Bind(wxEVT_MENU, [this, bambu_studio_id_base](wxEvent& event) {
        //    switch (event.GetId() - bambu_studio_id_base) {
        //        case VerslicerMenuAbout:
        //            Slic3r::GUI::about();
        //            break;
        //        case VerslicerMenuPreferences:
        //            CallAfter([this] {
        //                PreferencesDialog dlg(this);
        //                dlg.ShowModal();
        //#if ENABLE_GCODE_LINES_ID_IN_H_SLIDER
    ...
    // VFA — settings dialog disabled (interrupts normal slicing / AI assistant workflow).
    ...
    append_menu_item(m_topbar->GetCalibMenu(), wxID_ANY, _L("Calibration Guide"), _L("Calibration Guide"), [this](wxCommandEvent &)
                     { wxLaunchDefaultBrowser("https:///wiki/calibration_guide", wxBROWSER_NEW_WINDOW); }, "", nullptr, [this]()
    ...
    append_menu_item(calib_menu, wxID_ANY, _L("Calibration Guide"), _L("Calibration Guide"),
        [this](wxCommandEvent&) { wxLaunchDefaultBrowser("https:///wiki/calibration_guide", wxBROWSER_NEW_WINDOW); }, "", nullptr,
        [this]() {return m_plater->is_view3D_shown();; }, this);

    m_menubar->Append(calib_menu,wxString::Format("&%s", _L("Calibration")));

    wxMenu* smart_print_menu = new wxMenu();
    append_menu_item(smart_print_menu, wxID_ANY, _L("Analyze current plate"), _L("Run Smart Print analysis on the current plate"),
        [](wxCommandEvent&) {
            if (wxGetApp().plater())
                BambuSmartPrintService::instance().analyze_current_plate(wxGetApp().plater());
        }, "", nullptr,
        []() { return wxGetApp().plater() != nullptr; }, this);
    append_menu_item(smart_print_menu, wxID_ANY, _L("Analyze all plates"), _L("Run Smart Print analysis on each plate with models"),
        [](wxCommandEvent&) {
            if (wxGetApp().plater())
                BambuSmartPrintService::instance().analyze_all_plates(wxGetApp().plater());
        }, "", nullptr,
        []() { return wxGetApp().plater() != nullptr; }, this);
    append_menu_item(smart_print_menu, wxID_ANY, _L("Smart slice"), _L("Apply Smart Print suggestions and slice"),
        [](wxCommandEvent&) {
            if (wxGetApp().plater())
                BambuSmartPrintService::instance().run_smart_slice(wxGetApp().plater());
        }, "", nullptr,
        []() { return wxGetApp().plater() != nullptr; }, this);
    smart_print_menu->AppendSeparator();
    append_menu_item(smart_print_menu, wxID_ANY, _L("Sign in to Bambu Lab…"), _L("Log in with your Bambu Lab account to sync printers"),
        [](wxCommandEvent&) { BambuSmartPrintService::instance().prompt_bambu_login(wxGetApp().plater()); },
        "", nullptr, []() { return true; }, this);
    append_menu_item(smart_print_menu, wxID_ANY, _L("Sign out of Bambu Lab"), _L("Log out of your Bambu Lab account"),
        [](wxCommandEvent&) { BambuSmartPrintService::instance().prompt_bambu_logout(); },
        "", nullptr,
        []() { return BambuSmartPrintService::instance().is_bambu_account_logged_in(); }, this);
    smart_print_menu->AppendSeparator();
    append_menu_item(smart_print_menu, wxID_ANY, _L("View failure history"), _L("Browse recorded print failures and feedback"),
        [](wxCommandEvent&) { BambuSmartPrintService::instance().show_history_dialog(wxGetApp().plater()); },
        "", nullptr, []() { return true; }, this);
    smart_print_menu->AppendSeparator();
    append_menu_item(smart_print_menu, wxID_ANY, _L("Privacy and data…"), _L("Export or delete local Smart Print data"),
        [](wxCommandEvent&) { BambuSmartPrintService::instance().show_privacy_dialog(wxGetApp().plater()); },
        "", nullptr, []() { return true; }, this);
    append_menu_item(smart_print_menu, wxID_ANY, _L("About Smart Print scope…"),
        _L("What Smart Print supports (Bambu Lab printers only)"),
        [](wxCommandEvent&) {
            wxString msg = BambuSmartPrintService::bbl_printer_required_message();
            msg << "\n\n" << _L("Smart Print does not call cloud AI. Data stays on this PC unless you sign in to Bambu Lab for printer sync.");
            wxMessageBox(msg, _L("Smart Print"),
                         wxOK | wxICON_INFORMATION, wxGetApp().plater());
        },
        "", nullptr, []() { return true; }, this);
    m_menubar->Append(smart_print_menu, wxString::Format("&%s", _L("Smart Print")));
...
bool MainFrame::open_menubar_item(const wxString& menu_name,const wxString& item_name)
{
    if (m_menubar == nullptr)
        return false;
    // Get menu object from menubar
    const int menu_index = m_menubar->FindMenu(menu_name);
    if (menu_index == wxNOT_FOUND) {
...
void MainFrame::jump_to_smart_print()
{
    if (!m_smart_print_page)
        return;
    select_tab(m_smart_print_page);
    if (m_smart_print_panel)
        m_smart_print_panel->refresh_all();
}
...
void MainFrame::on_select_default_preset(SimpleEvent& evt)
{
    MessageDialog dialog(this,
                    _L("Do you want to synchronize your personal data from Cloud?\n"
...
#else
    SetIcon(wxIcon(var("Verslicer.ico"), wxBITMAP_TYPE_ICO));
#endif // _WIN32
