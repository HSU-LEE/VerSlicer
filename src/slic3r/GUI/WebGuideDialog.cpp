...
#include "slic3r/GUI/BambuSmartPrint/BambuSmartPrintService.hpp"
#include "slic3r/GUI/AICoach/BeginnerJourney.hpp"
...
GuideFrame::GuideFrame(GUI_App *pGUI, long style)
    : DPIDialog((wxWindow *) (pGUI->mainframe), wxID_ANY, SLIC3R_APP_FULL_NAME, wxDefaultPosition, wxDefaultSize, style),
	m_appconfig_new()
{
    ...
    // UI — load the real wizard page directly (never guide/0 "Loading……").
    if (std::string(SLIC3R_APP_FULL_NAME) == std::string("Verslicer"))
        SetStartPage(BBL_WELCOME, true);
    else
        SetStartPage(BBL_REGION);

    ensure_profile_load_started();
    if (m_browser) {
        m_browser->Show();
        Layout();
    }
...
{
    m_destroy = true;
    if (m_profile_load_fallback_timer) {
        m_profile_load_fallback_timer->Stop();
        delete m_profile_load_fallback_timer;
        m_profile_load_fallback_timer = nullptr;
    }
    if (m_plugin_progress) {
        delete m_plugin_progress;
        m_plugin_progress = nullptr;
    }
...
wxString GuideFrame::build_guide_page_url(bool with_file_scheme)
{
    int page_idx = 1;
    switch (m_page) {
    case BBL_WELCOME: page_idx = 1; break;
    case BBL_REGION: page_idx = 11; break;
    case BBL_MODELS: page_idx = 21; break;
    case BBL_FILAMENTS:
        page_idx = m_ProfileJson.contains("model") && m_ProfileJson["model"].is_array() && !m_ProfileJson["model"].empty() ? 22 : 21;
        break;
    case BBL_FILAMENT_ONLY: page_idx = 23; break;
    case BBL_MODELS_ONLY: page_idx = 24; break;
    default: page_idx = 21; break;
    }

    switch (m_page) {
    case BBL_WELCOME: SetTitle(_L("Setup Wizard")); break;
    case BBL_REGION: SetTitle(_L("Setup Wizard")); break;
    case BBL_MODELS: SetTitle(_L("Setup Wizard")); break;
    case BBL_FILAMENTS: SetTitle(_L("Setup Wizard")); break;
    case BBL_FILAMENT_ONLY:
    case BBL_MODELS_ONLY: SetTitle(""); break;
    default: SetTitle(_L("Setup Wizard")); break;
    }

    const auto path = (boost::filesystem::path(resources_dir()) / "web" / "guide" / std::to_string(page_idx) / "index.html").make_preferred();
    wxString url = from_u8(path.string());

    const wxString strlang = wxGetApp().current_language_code_safe();
    if (!strlang.empty())
...
void GuideFrame::navigate_away_from_loading_page()
{
    if (m_destroy || !m_browser)
        return;
    wxString url = build_guide_page_url(true);
    BOOST_LOG_TRIVIAL(info) << "GuideFrame::navigate_away_from_loading_page -> " << url.ToUTF8();
    load_url(url);
}

void GuideFrame::ensure_profile_load_started()
{
    if (bFirstComplete || m_destroy)
        return;
    start_profile_load_fallback_timer();
...
wxString GuideFrame::SetStartPage(GuidePage startpage, bool load)
{
    ...
    wxString target_url = build_guide_page_url(true);
    if (load)
        load_url(target_url);

    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " exit, url=" << target_url.ToUTF8();
    return target_url;
}
 ...
 */
void GuideFrame::start_profile_load_fallback_timer()
{
    if (m_profile_load_fallback_timer || m_destroy)
        return;

    m_profile_load_fallback_timer = new wxTimer(this);
    Bind(wxEVT_TIMER, [this](wxTimerEvent&) {
        if (m_profile_load_fallback_timer)
            m_profile_load_fallback_timer->Stop();
        BOOST_LOG_TRIVIAL(warning) << "GuideFrame: profile load fallback timeout — continuing setup wizard";
        notify_profile_load_finish();
    }, m_profile_load_fallback_timer->GetId());
    m_profile_load_fallback_timer->StartOnce(8000);
}

bool GuideFrame::guide_page_needs_profile(const wxString &url) const
{
    return url.Contains("/guide/21/") || url.Contains("/guide/22/") || url.Contains("/guide/23/") ||
           url.Contains("/guide/24/") || url.Contains("guide%2F21") || url.Contains("guide%2F22") ||
           url.Contains("guide%2F23") || url.Contains("guide%2F24");
}

void GuideFrame::push_userguide_profile_to_web()
{
    if (m_destroy || !m_browser || !m_profile_data_ready)
        return;
    ...
    wxGetApp().CallAfter([this]() {
        ...
        const wxString cur = m_browser->GetCurrentURL();
        if (!guide_page_needs_profile(cur))
            return;
        ...
        const wxString strJS     = wxString::Format("HandleStudio(%s)", m_Res.dump(-1, ' ', true));
        const size_t model_count = m_ProfileJson.contains("model") && m_ProfileJson["model"].is_array()
                                       ? m_ProfileJson["model"].size()
                                       : 0;
        BOOST_LOG_TRIVIAL(info) << "GuideFrame::push_userguide_profile_to_web, models=" << model_count;
        RunScript(strJS);
    });
}

void GuideFrame::notify_profile_load_finish()
{
    if (m_profile_load_finished_sent || m_destroy)
        return;
    m_profile_load_finished_sent = true;
    ...
    wxGetApp().CallAfter([this]() {
        ...
        const wxString cur = m_browser->GetCurrentURL();
        // Legacy splash page — navigate in C++ (WKWebView often ignores JS window.open).
        if (cur.Contains("/guide/0/") || cur.Contains("guide%2F0") || cur.Contains("guide/0/index")) {
            navigate_away_from_loading_page();
            return;
        }
        ...
        const wxString strJS   = wxString::Format("HandleStudio(%s)", res.dump(-1, ' ', true));
        BOOST_LOG_TRIVIAL(info) << "GuideFrame::notify_profile_load_finish (script)";
        RunScript(strJS);
        push_userguide_profile_to_web();
    });
}
...
void GuideFrame::OnDocumentLoaded(wxWebViewEvent &evt)
{
    ...
    // Printer/filament wizard pages request profile on load; re-push when data is ready
    // (avoids empty list if RequestProfile() ran before LoadProfileData finished).
    if (guide_page_needs_profile(evt.GetURL()) && m_profile_data_ready)
        push_userguide_profile_to_web();
}
...
void GuideFrame::OnScriptMessage(wxWebViewEvent &evt)
{
    try {
        ...
        else if (strCmd == "save_userguide_models")
        {
            ...
            if (InstallNetplugin && std::string(SLIC3R_APP_FULL_NAME) != std::string("Verslicer"))
...
#define WX_ERROR_CASE(type) \
    ...
    BOOST_LOG_TRIVIAL(warning) << "GuideFrame::OnError: An error occurred loading " << evt.GetURL() << " (" << category << ")";

    // Do not leave the wizard stuck on guide/0 "Loading……" if local assets fail to load.
    notify_profile_load_finish();
...
bool GuideFrame::run()
{
    ...
    if (result == wxID_OK) {
        ...
        BeginnerJourney::on_printer_configured();
        this->Close();
        return true;
    } else if (result == wxID_CANCEL) {
        BOOST_LOG_TRIVIAL(info) << "GuideFrame cancelled";
        // Do not reopen the wizard on every launch after the user dismisses it.
        app.app_config->set("firstguide", "finish", "1");
        app.app_config->save();
        {
            const bool plugin_ok = wxGetApp().ensure_bambu_network_plugin_installed(false);
            const bool bbl = app.preset_bundle && SlicePilot::is_active_printer_bbl(*app.preset_bundle);
            wxString checklist = _L("Setup was not finished. Smart Print may be limited until you complete:\n\n");
            checklist << (bbl ? wxString::FromUTF8("✓ ") : wxString::FromUTF8("○ "))
                      << _L("Bambu Lab printer profile selected\n");
            checklist << (plugin_ok ? wxString::FromUTF8("✓ ") : wxString::FromUTF8("○ "))
                      << _L("Network plug-in installed (for printer connection & failure learning)\n");
            checklist << (BambuSmartPrintService::is_enabled() ? wxString::FromUTF8("✓ ") : wxString::FromUTF8("○ "))
                      << _L("Smart Print enabled (Preferences or Smart Print tab)\n\n");
            checklist << _L("You can finish setup later from Help → Setup Wizard.");
            wxMessageDialog hint(this, checklist, _L("Smart Print readiness"), wxOK | wxICON_INFORMATION);
            hint.ShowModal();
        }
        if (app.preset_bundle->printers.only_default_printers()) {
            ...
            if (std::string(SLIC3R_APP_FULL_NAME) == std::string("Verslicer")) {
                if (!BambuSmartPrintService::try_activate_bbl_printer_profile(plater)) {
                    for (const Preset& preset : app.preset_bundle->printers) {
                        if (!SlicePilot::is_bbl_printer_preset(preset, app.preset_bundle))
                            continue;
                        if (!preset.is_visible || !preset.is_compatible)
                            continue;
                        app.preset_bundle->printers.select_preset_by_name(preset.name, true);
                        break;
                    }
                }
                if (plater)
                    plater->on_config_change(app.preset_bundle->full_config(false));
            } else {
    ...
    try {
        ...
        for (boost::filesystem::directory_iterator iter(vendor_dir); iter != endIter; iter++) {
            if (!boost::filesystem::is_directory(*iter)) {
                ...
                if (!Slic3r::SlicePilot::is_vendor_allowed_for_slicepilot(vendor_name))
                    continue;
...
{
    if (status == InstallStatusDownloadFailed || status == InstallStatusUnzipFailed) {
        if (m_plugin_progress) {
            m_plugin_progress->Update(0, _L("Network plug-in installation failed. Install later under Preferences → Online."));
            delete m_plugin_progress;
            m_plugin_progress = nullptr;
        }
        return 0;
    ...
    if (!m_plugin_progress) {
        m_plugin_progress = new wxProgressDialog(
            _L("Network plug-in"),
            _L("Required for Bambu printer connection and Smart Print failure learning."),
            100, this, wxPD_APP_MODAL | wxPD_AUTO_HIDE | wxPD_CAN_ABORT);
    }

    wxString step = percent < 50
        ? _L("Downloading network plug-in…")
        : _L("Installing network plug-in…");
    if (!m_plugin_progress->Update(std::min(100, std::max(0, percent)), step, &cancel)) {
        cancel = true;
    }

    if (status == InstallStatusInstallCompleted || percent >= 100) {
        delete m_plugin_progress;
        m_plugin_progress = nullptr;
    }
    return 0;
