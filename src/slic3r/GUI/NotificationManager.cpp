	...
	, m_notification_start  (GLCanvas3D::timestamp_now())
{
    ...
    m_NormalColor = ImGuiWrapper::to_ImVec4(decode_color_to_float_array("#FF8F4A")); // Verslicer orange

	m_CurrentColor = m_NormalColor;   //Default

	m_WindowBkgColor = ImVec4(1, 1, 1, 1);
    m_TextColor      = ImVec4(.2f, .2f, .2f, 1.0f);
    m_HyperTextColor = m_NormalColor;
}
...
void NotificationManager::PopNotification::use_bbl_theme()
{
 ...
 //   OldStyle.Colors[ImGuiCol_Text]     = m_TextColor;
	...
	m_HyperTextColor = m_NormalColor;
	m_HyperTextColorHover = m_is_dark ? ImGuiWrapper::to_ImVec4(decode_color_to_float_array("#E67A2E"))
	                                  : ImGuiWrapper::to_ImVec4(decode_color_to_float_array("#FFB07A"));
...
void NotificationManager::PopNotification::render_close_button(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
	...
	push_style_color(ImGuiCol_TextSelectedBg, ImGuiWrapper::to_ImVec4(decode_color_to_float_array("#FFE0CC")), m_state == EState::FadingOut, m_current_fade_opacity);
