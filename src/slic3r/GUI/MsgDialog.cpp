                            ...
                            std::function<void(const wxString &)> link_callback = nullptr)
{
    ...
    if (!link_text.IsEmpty() && link_callback) {
        msg_escaped += "<span><a href=\"#\" style=\"color:rgb(255, 143, 74); text-decoration:underline;\">" + std::string(link_text.ToUTF8().data()) + "</a></span>";
    }
    ...
    , msg(temp_msg)
{
	...
	logo->SetBitmap(create_scaled_bitmap("Verslicer_192px_grayscale.png", this, monospaced_font ? 48 : /*1*/64));
...
wxBoxSizer *Newer3mfVersionDialog::get_msg_sizer()
{
    ...
    if (file_version_newer) { 
        ...
        text1 = new wxStaticText(this, wxID_ANY, _L("The 3MF file version is newer than the current Verslicer version."));
        wxStaticText *text2 = new wxStaticText(this, wxID_ANY, _L("Updating your Verslicer could enable all functionality in the 3MF file."));
        horizontal_sizer->Add(text2, 0, wxEXPAND, 0);
    }
