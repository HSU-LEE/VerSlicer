...
void GLCanvas3D::render(bool only_init)
{
    ...
    // draw scene — drain stale errors from picking / ImGui before strict glsafe(glClear)
#ifndef NDEBUG
    ...
    if (m_canvas_type != ECanvasType::CanvasAssembleView) {
        ...
        if (AIGuiOrchestrator::instance().should_render_dailytips_window())
...
void GLCanvas3D::on_idle(wxIdleEvent& evt)
{
    ...
    if (wxGetApp().plater() && wxGetApp().plater()->get_current_canvas3D() == this) {
        static int64_t s_last_coach_tick = 0;
    ...
    if (Plater* plater = wxGetApp().plater()) {
        if (GLCanvas3D* view3d = plater->get_view3D_canvas3D()) {
            if (auto* gizmo = view3d->get_gizmos_manager().get_current())
...
void GLCanvas3D::_render_imgui_select_plate_toolbar()
{
    ...
    ImVec4 button_active = m_is_dark ? ImVec4(0.50f, 0.50f, 0.52f, 1.f) : ImVec4(0.60f, 0.60f, 0.62f, 1.f);
    ...
    ImU32  plate_dim     = ImGui::ColorConvertFloat4ToU32(m_is_dark ? ImVec4(0.20f, 0.20f, 0.22f, 0.75f) : ImVec4(0.88f, 0.88f, 0.90f, 0.85f));
    ...
    if(m_sel_plate_toolbar.show_stats_item)
    {
        ...
        if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::UNSLICED || all_plates_stats_item->slice_state == IMToolbarItem::SliceState::SLICING) {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(bar_bg_bgn, bar_fg_end, IM_COL32(255, 143, 74, 255),  bar_size.y); // BAR PROGRESS
        }
        else if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::SLICE_FAILED) {
    ...
    for (int i = 0; i < m_sel_plate_toolbar.m_items.size(); i++) {
        ...
        // Some builds may not have a generated plate preview texture yet; avoid rendering a black quad.
        const bool has_tex = item->texture_id != (ImTextureID)0;
        if (has_tex) {
            ImGui::Image(item->texture_id, size, uv0, uv1, tint_col);
        } else {
            ImVec2 p0 = ImGui::GetCursorScreenPos();
            ImVec2 p1 = p0 + size;
            ImGui::GetWindowDrawList()->AddRectFilled(p0, p1, plate_dim, button_radius);
        }
...
void GLCanvas3D::_render_canvas_toolbar() 
{
    ...
    // Ollama chat toggle (KakaoTalk-like floating chat window)
    ImGui::Dummy({ 0, spacing.y});

    ImTextureID c_normal_id = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_CHAT_DARK       : GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_CHAT);
    ImTextureID c_hover_id  = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_CHAT_DARK_HOVER : GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_CHAT_HOVER);
    if (ImGui::ImageButton3(c_normal_id, c_hover_id, btn_size)) {
        if (m_canvas != nullptr && !wxGetApp().is_closing()) {
            m_canvas->CallAfter([]() {
                auto& app = wxGetApp();
        ...
        auto tooltip_str_wx = _L("Open Ollama chat");
    ...
    // Mic input (speech-to-text) button below chat.
    ImGui::Dummy({ 0, spacing.y});
    ImTextureID mic_normal_id = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MIC_DARK       : GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MIC);
    ImTextureID mic_hover_id  = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MIC_DARK_HOVER : GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MIC_HOVER);
    if (ImGui::ImageButton3(mic_normal_id, mic_hover_id, btn_size)) {
        if (m_canvas != nullptr && !wxGetApp().is_closing()) {
            m_canvas->CallAfter([]() {
                auto& app = wxGetApp();
                if (app.is_closing() || app.mainframe == nullptr)
                ...
                if (!app.ollama_voice_input) {
                    app.ollama_voice_input = create_ollama_voice_input();
                    if (app.ollama_voice_input) {
                        app.ollama_voice_input->set_on_error([](const std::string& err) {
                            wxGetApp().CallAfter([err] {
                                ...
                                if (wxGetApp().plater() && wxGetApp().plater()->get_notification_manager()) {
                                    wxGetApp().plater()->get_notification_manager()->push_notification(
                                        ...
                                        std::string(_u8L("Voice input error: ")) + err
                                    );
                                }
                            });
                        });
                ...
                if (app.ollama_voice_input) {
                    if (app.ollama_voice_input->is_listening())
                        app.ollama_voice_input->stop();
                    else
                        app.ollama_voice_input->start();
                    if (app.plater() && app.plater()->get_notification_manager()) {
                        app.plater()->get_notification_manager()->push_notification(
                            NotificationType::CustomNotification,
                            NotificationManager::NotificationLevel::RegularNotificationLevel,
                            app.ollama_voice_input->is_listening()
                                ? _u8L("Listening… Tap mic again when done speaking.")
                                : _u8L("Processing voice…")
                        );
                    }
                    ...
                    app.plater()->get_notification_manager()->push_notification(
                        NotificationType::CustomNotification,
                        NotificationManager::NotificationLevel::WarningNotificationLevel,
                        _u8L("Voice input is only available on macOS.")
                    );
                }
            });
        }
    } else if (ImGui::IsItemHovered()) {
        auto tooltip_str_wx = _L("Voice input (speech)");
        std::string tooltip_str = tooltip_str_wx.ToUTF8().data();
        float width = ImGui::CalcTextSize(tooltip_str.c_str()).x + imgui.scaled(2.0f);
        imgui.tooltip(tooltip_str, width);
    }
