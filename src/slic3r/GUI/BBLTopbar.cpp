...
wxColour topbar_background()
{
    return StateColor::darkModeColorFor(wxColour("#3B4446"));
}

wxColour topbar_title_text()
{
    return StateColor::darkModeColorFor(wxColour("#ACACAC"));
}
    ...
    : wxControl()
{
    ...
    Bind(wxEVT_PAINT, [this](wxPaintEvent&) {
        ...
        dc.SetBackground(wxBrush(topbar_background()));
        dc.Clear();

        dc.SetTextForeground(topbar_title_text());
...
{
    dc.SetBrush(wxBrush(topbar_background()));
...
void BBLTopbarArt::DrawButton(wxDC& dc, wxWindow* wnd, const wxAuiToolBarItem& item, const wxRect& rect)
{
    ...
    if (!(item.GetState() & wxAUI_BUTTON_STATE_DISABLED))
    {
        ...
        {
            dc.SetPen(wxPen(StateColor::darkModeColorFor("#FF8F4A"))); // ORCA
            dc.SetBrush(wxBrush(StateColor::darkModeColorFor("#FF8F4A"))); // ORCA
            dc.DrawRectangle(rect);
        }
