...
private:
    ...
    bool m_profile_load_finished_sent{false};
    bool m_profile_data_ready{false};
    boost::thread* m_load_task{ nullptr };
    wxTimer* m_profile_load_fallback_timer{nullptr};

    void notify_profile_load_finish();
    void push_userguide_profile_to_web();
    bool guide_page_needs_profile(const wxString &url) const;
    void start_profile_load_fallback_timer();
    void ensure_profile_load_started();
    wxString build_guide_page_url(bool with_file_scheme);
    void navigate_away_from_loading_page();
    ...
    wxProgressDialog* m_plugin_progress{ nullptr };
