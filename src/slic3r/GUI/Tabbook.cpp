...
static wxColour TAB_BUTTON_BG()  { return StateColor::darkModeColorFor(wxColour("#FEFFFF")); }
static wxColour TAB_BUTTON_SEL() { return StateColor::darkModeColorFor(wxColour("#FFE0CC")); } // ORCA
...
void TabButtonsListCtrl::SetSelection(int sel)
{
    if (m_selection == sel)
        return;
    wxFont normal_font = TAB_BUTTON_FONT;
    wxFont selected_font = TAB_BUTTON_FONT;
    selected_font.SetWeight(wxFONTWEIGHT_BOLD);

    if (m_selection >= 0) {
        m_pageButtons[m_selection]->SetBackgroundColor(TAB_BUTTON_BG());
        m_pageButtons[m_selection]->SetFont(normal_font);
    }
    m_selection = sel;
    m_pageButtons[m_selection]->SetBackgroundColor(TAB_BUTTON_SEL());
    m_pageButtons[m_selection]->SetFont(selected_font);
    for (TabButton *btn : m_pageButtons)
        btn->Refresh();
    Refresh();
}
