...
static bool ensure_bambu_network_ready_for_jobs()
{
    ...
    if (!bundle->use_bbl_network()) {
        wxMessageBox(
            _L("Select a Bambu Lab printer in the top toolbar (for example X1 Carbon or P1S).\n\n"
               "A custom/Klipper print-host preset cannot send jobs through Bambu LAN or cloud."),
            _L("Bambu Lab printer required"),
            wxOK | wxICON_WARNING);
        return false;
    }
    ...
    if (!wxGetApp().app_config || !wxGetApp().app_config->get_bool("installed_networking")) {
        wxMessageBox(
            _L("Bambu network plug-in is disabled.\n\n"
               "Open Preferences → Online, enable \"Enable network plug-in\", install it, then restart."),
            _L("Network plug-in required"),
            wxOK | wxICON_WARNING);
        wxGetApp().ShowDownNetPluginDlg();
        return false;
    }

    if (!Slic3r::NetworkAgent::is_network_module_loaded() || !wxGetApp().getAgent()) {
        if (!wxGetApp().ensure_bambu_network_plugin_installed(true)
            && (!Slic3r::NetworkAgent::is_network_module_loaded() || !wxGetApp().getAgent())) {
            wxMessageBox(
                wxString::Format(
                    _L("Bambu network plug-in is not loaded (error -1).\n\n"
                       "Allow the download at startup or install it under Preferences → Online, then restart %s."),
...
static void switch_to_preview_tab_after_slice(Plater* plater)
{
    if (!plater)
        return;
    MainFrame* mf = wxGetApp().mainframe;
    if (!mf || !mf->m_tabpanel || mf->m_tabpanel->GetSelection() != MainFrame::tp3DEditor)
        return;
    // One-click print jumps straight to send — avoid flashing Preview first.
    if (BambuSmartPrintService::instance().one_click_phase() == BambuSmartPrintService::OneClickPhase::Exporting)
        return;
    plater->select_view_3D("Preview", true);
...
void Sidebar::priv::layout_printer(bool isBBL, bool isDual)
{
    ...
    // ref: https://github.com/Verslicer/Verslicer/pull/11610#discussion_r2607411847
...
void Sidebar::priv::flush_printer_sync(bool restart)
{
    ...
    //btn_sync_printer->SetBackgroundColorNormal((*counter_sync_printer & 1) ? "#F8F8F8" :"#FF8F4A");
...
{
    StateColor not_synced_colour(std::pair<wxColour, int>(wxColour("#FF8F4A"), StateColor::Normal));
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(39 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        p->m_printer_connect->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) {
        ...
        struct PanelColors {
            ...
            wxColour bg_focus  = "#FFF0E5";
            wxColour bd_normal = "#DBDBDB";
            wxColour bd_hover  = "#FF8F4A";
            wxColour bd_focus  = "#FF8F4A";
        };
        ...
        */
        ...
        StateColor btn_sync_bd_col(
                std::pair<wxColour, int>(wxColour("#FF8F4A"), StateColor::Pressed),
                std::pair<wxColour, int>(wxColour("#FF8F4A"), StateColor::Hovered),
    ...
    text_ctrl->Bind(wxEVT_SET_FOCUS, [this](wxFocusEvent& e) {
        ...
        p->m_search_bar->SetBorderColor(wxColour("#FF8F4A"));
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
            _L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update Verslicer or restart Verslicer to check if there is an update to system presets.") + detail,
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    if (!sync_color_only) {
...
std::vector<size_t> Plater::priv::load_files(const std::vector<fs::path>& input_files, LoadStrategy strategy, bool ask_multi)
{
    ...
    // Verslicer: ugly fix so we can exist pa calib mode
        ...
        try {
            if (type_3mf) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    const wxString msg_unsupported_geometry    = _L("The 3MF is not supported by Verslicer, loading geometry data only.");
                    const wxString msg_old_orca_geometry       = _L("The 3MF file was generated by an old Verslicer version, loading geometry data only.");
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        // Verslicer file (has Verslicer tag) - compare file_version with SoftFever_VERSION
                        // Migration fix for Verslicer 2.3.1-alpha sparse infill rotation template
                        if (load_config && (file_version < app_version) && file_version == Semver("2.3.1-alpha")) {
                            if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                                ...
                                if (!is_safe_to_rotate) {
                                    wxString msg_text = _(
                                        L("This project was created with an Verslicer 2.3.1-alpha and uses "
                                    ...
                                    msg_text += "\n\n" +
                                                _(L("Would you like Verslicer to automatically fix this by clearing the rotation template settings?"));
                        ...
                        // No Verslicer tag - check Bambu/Application version
                        Semver orca_tag_start_version(2, 3, 2);
                        if (file_version <= orca_tag_start_version) {
                            // Compatible old version (before Verslicer tagging was introduced after 2.3.2).
                            ...
                            // BambuStudio project (version > 2.3.2 without Verslicer tag)
                            ...
                            if (load_config && config_loaded.empty()) {
                                ...
                                wxString text = _L("The 3MF was created by BambuStudio. Some settings may differ from Verslicer.");
                                log_and_show_3mf_info(text, bambu_project_title);
                            }
                        }
                    }
...
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs& model_objects, bool allow_negative_z, bool split_object, bool auto_drop)
{
    ...
    for (auto& instance : new_instances) {
        ...
        if (plate_empty)
            ...
            Vec2d empty_cell{start_point(0), start_point(1)};
            if (GLCanvas3D* canvas = wxGetApp().plater()->canvas3D()) {
                const Vec2f cell_f = canvas->get_nearest_empty_cell({start_point(0), start_point(1)});
                empty_cell = cell_f.cast<double>();
            }
...
void Plater::priv::export_gcode(fs::path output_path, bool output_path_on_removable_media)
{
    ...
    if (wxGetApp().preset_bundle && !Slic3r::SlicePilot::is_active_printer_bbl(*wxGetApp().preset_bundle)) {
        Slic3r::GUI::show_error(q, wxString::Format(_L("%s supports Bambu Lab printers only. Please select a Bambu Lab printer profile."), SLIC3R_APP_FULL_NAME));
        return;
    }
...
void Plater::priv::on_select_preset(wxCommandEvent &evt)
{
    ...
    if (preset_type == Preset::TYPE_FILAMENT && sidebar->is_multifilament()) {
        ...
        if (preset_type == Preset::TYPE_PRINTER) {
            ...
            {
                const Preset* printer_preset = wxGetApp().preset_bundle->printers.find_preset(preset_name);
                if (printer_preset && !Slic3r::SlicePilot::is_bbl_printer_preset(*printer_preset, wxGetApp().preset_bundle)) {
                    Slic3r::GUI::show_error(q, wxString::Format(_L("%s supports Bambu Lab printers only. Please select a Bambu Lab printer profile."), SLIC3R_APP_FULL_NAME));
                    combo->update();
                    return;
                }
...
void Plater::priv::on_process_completed(SlicingProcessCompletedEvent &evt)
{
    ...
    if (is_finished && this->printer_technology == ptFFF && this->background_process.m_fff_print) {
        const bool slice_ok = evt.success() && !has_error && !evt.cancelled();
        GUI::BambuSmartPrintService::instance().on_slice_completed(
            this->q, this->background_process.m_fff_print, slice_ok);
        GUI::AIGuiOrchestrator::instance().on_slice_completed(
            this->q, this->background_process.m_fff_print, slice_ok);
        if (slice_ok) {
...
void Plater::priv::on_action_print_plate(SimpleEvent&)
{
    ...
    if (!ensure_bambu_network_ready_for_jobs())
        return;
...
void Plater::send_print_job_for_plate(int plate_index, bool auto_send)
{
    wxGetApp().CallAfter([this, plate_index, auto_send]() {
        if (!p)
            return;
        if (!ensure_bambu_network_ready_for_jobs())
            return;
        ...
        if (preset_bundle.use_bbl_network()) {
            if (!p->m_select_machine_dlg)
                p->m_select_machine_dlg = new SelectMachineDialog(this);
            p->m_select_machine_dlg->set_auto_send_on_ready(auto_send);
            p->m_select_machine_dlg->set_print_type(PrintFromType::FROM_NORMAL);
            p->m_select_machine_dlg->prepare(plate_index);
            p->m_select_machine_dlg->ShowModal();
        } else {
            Slic3r::GUI::show_error(this, _L("Install the Bambu network plugin to send print jobs."));
        }
    });
}
...
#else
    wxGetApp().mainframe->SetTitle(m_project_name + wxString(" - ") + SLIC3R_APP_FULL_NAME);
    wxGetApp().mainframe->topbar()->SetTitle(m_project_name.IsEmpty() ? SLIC3R_APP_FULL_NAME : m_project_name);
#endif
}
...
void Plater::priv::update_title_dirty_status()
{
    ...
    wxGetApp().mainframe->topbar()->SetTitle(title.IsEmpty() ? SLIC3R_APP_FULL_NAME : title);
#endif    
}
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
    ...
    if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
        ...
        if (printer_technology_changed) {
            ...
            Slic3r::SlicePilot::enforce_bbl_only_bundle(*wxGetApp().preset_bundle);
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Verslicer window going forward";
    ...
    wxString const& originfile)
{
    ...
    m_loading_project = false;
    GUI::OllamaActionExecutor::notify_plater_context_changed(true);
}
...
void Plater::import_model_id(wxString download_info)
{
    ...
    try
    {
        ...
        if (namePos != wxString::npos) {
            ...
            filename     = wxString::FromUTF8(
                Slic3r::Http::url_decode(download_info.Mid(namePos + separator.Length()).utf8_string()));
        }
        else {
...
//        if (!m_agent) return;
        ...
        if (MakerWorldSearchService::download_url_needs_auth(download_url.ToStdString()))
            MakerWorldSearchService::apply_download_http_headers(http);
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to Verslicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
...
int GUI::Plater::close_with_confirm(std::function<bool(bool)> second_check)
{
    ...
    MessageDialog dlg(static_cast<wxWindow*>(this),
        _L("This project has unsaved changes. Do you want to save before continuing?"),
        wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Save"),
        wxYES_NO | wxCANCEL | wxYES_DEFAULT | wxCENTRE | wxICON_QUESTION);
    dlg.show_dsa_button(_L("Remember my choice."));
...
bool Plater::export_gcode_to_path(const std::string& path)
{
    ...
    if (wxGetApp().preset_bundle && !Slic3r::SlicePilot::is_active_printer_bbl(*wxGetApp().preset_bundle)) {
        Slic3r::GUI::show_error(this, wxString::Format(_L("%s supports Bambu Lab printers only. Please select a Bambu Lab printer profile."), SLIC3R_APP_FULL_NAME));
        return false;
    }
    ...
    if (output_path.has_parent_path()) {
        boost::system::error_code ec;
        boost::filesystem::create_directories(output_path.parent_path(), ec);
    }

    AppConfig& appconfig = *wxGetApp().app_config;
    RemovableDriveManager& removable_drive_manager = *wxGetApp().removable_drive_manager();
    const bool path_on_removable_media = removable_drive_manager.set_and_verify_last_save_path(output_path.string());
...
{
    Slic3r::GUI::show_error(this,
        wxString::Format(_L("%s sends jobs to Bambu Lab printers via the network plugin. Use Send to Printer."), SLIC3R_APP_FULL_NAME));
}
...
void Plater::on_config_change(const DynamicPrintConfig &config)
{
    ...
    bool clear_coach_dedup = false;
    if (PresetBundle* bundle = wxGetApp().preset_bundle) {
        static std::string              last_print;
        static std::string              last_printer;
        static std::vector<std::string> last_filaments;
        static bool                     preset_tracker_initialized = false;

        const std::string              cur_print     = bundle->prints.get_selected_preset_name();
        const std::string              cur_printer   = bundle->printers.get_selected_preset_name();
        const std::vector<std::string> cur_filaments = bundle->filament_presets;

        if (preset_tracker_initialized &&
            (cur_print != last_print || cur_printer != last_printer || cur_filaments != last_filaments))
            clear_coach_dedup = true;

        last_print     = cur_print;
        last_printer   = cur_printer;
        last_filaments = cur_filaments;
        preset_tracker_initialized = true;
    }

    t_config_option_keys diff_keys = p->config->diff(config);

    if (!diff_keys.empty())
        GUI::OllamaActionExecutor::notify_plater_context_changed(clear_coach_dedup);
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("Verslicer can't connect to %s. Please check if the printer is powered on and connected to the network."), printer_name);
        }
