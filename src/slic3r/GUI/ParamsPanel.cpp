    ...
    : wxPanel( parent, id, pos, size, style, name )
{
    ...
    // Allow the scroll area to shrink so fixed bottom widgets (like chat) remain visible.
    m_tmp_panel->SetMinSize(wxSize(-1, 0));
    ...
    // Allow shrinking so the bottom chat panel can stay visible.
    m_page_view->SetMinSize(wxSize(-1, 0));
