...
private:
    ...
    bool prefer_lan_liveview() const;

    bool can_use_lan_liveview() const;

    bool start_lan_play(const std::string& agent_version);

    bool try_lan_fallback(const std::string& agent_version, const wxString& failure_hint = {});

    void begin_cloud_play(const std::string& agent_version);

    void request_next_cloud_play(const std::string& agent_version);

    static wxString liveview_forbidden_user_message();
...
private:
    ...
    std::vector<int> m_pending_cloud_protos;
    wxString m_url;
