...
constexpr const char* SECRET_STORE_SERVICE = "Verslicer/Auth";
...
int OrcaCloudServiceAgent::search_makerworld(const std::string& query, int limit, const std::string& locale,
                                             ...
                                             unsigned int* http_code, std::string* http_error)
{
    ...
    (void)http_code;
    if (http_error)
        *http_error = "not available";
    return -1;
}

int OrcaCloudServiceAgent::get_makerworld_download_url(const std::string& design_id, std::string* download_url,
