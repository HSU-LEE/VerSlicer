...
public:
    ...
    /**
     * Search MakerWorld models; response is JSON with a "hits" array of model entries.
     */
    virtual int search_makerworld(const std::string& query, int limit, const std::string& locale,
                                const std::string& printer_model, std::string* json_body,
                                unsigned int* http_code, std::string* http_error) = 0;

    /**
     * Resolve direct 3MF download URL for a design id.
     */
    virtual int get_makerworld_download_url(const std::string& design_id, std::string* download_url,
