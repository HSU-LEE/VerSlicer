...
std::unique_ptr<NetworkAgent> create_agent_from_config(const std::string& log_dir, AppConfig* app_config)
{
    ...
    if (agent) {
        ...
        // Camera / bind / LAN features need BBL even when only Orca cloud is listed in preferences.
        if (app_config->get_bool("installed_networking") && !agent->get_cloud_agent(BBL_CLOUD_PROVIDER)) {
            ...
            if (bbl_agent) {
                ...
                BOOST_LOG_TRIVIAL(info) << "Initialized BBL cloud agent (network plugin)";
            } else {
                BOOST_LOG_TRIVIAL(warning) << "BBL network plugin not loaded; camera and cloud printer APIs unavailable";
            }
        }
    }
