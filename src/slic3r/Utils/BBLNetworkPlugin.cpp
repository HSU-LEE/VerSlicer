...
std::string BBLNetworkPlugin::get_libpath_in_current_directory(const std::string& library_name)
{
    ...
    auto try_replace_exe = [&](const char* exe_name, size_t exe_len) {
        std::size_t found = file_name_string.rfind(exe_name);
        if (found != std::string::npos && found + exe_len == file_name_string.size()) {
            lib_path = library_name + ".dll";
            lib_path = file_name_string.substr(0, found) + lib_path;
            return true;
        }
        return false;
    };
    if (!try_replace_exe("orca-slicer.exe", 16)
        && !try_replace_exe("slicepilot.exe", 14)
        && !try_replace_exe("SlicePilot.exe", 14)) {
        lib_path.clear();
    }
#else
...
void BBLNetworkPlugin::load_all_function_pointers()
{
    ...
    m_search_makerworld = reinterpret_cast<func_search_makerworld>(get_function("bambu_network_search_makerworld"));
    m_get_makerworld_download_url =
        reinterpret_cast<func_get_makerworld_download_url>(get_function("bambu_network_get_makerworld_download_url"));
...
void BBLNetworkPlugin::clear_all_function_pointers()
{
    ...
    m_search_makerworld = nullptr;
    m_get_makerworld_download_url = nullptr;
