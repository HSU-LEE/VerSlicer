...
{
    // Live view is implemented by the Bambu network plugin; Orca cloud is a stub.
    if (const auto bbl_agent = get_cloud_agent(BBL_CLOUD_PROVIDER)) {
        return bbl_agent->get_camera_url(std::move(dev_id), std::move(callback));
    }
...
int NetworkAgent::search_makerworld(const std::string& query, int limit, const std::string& locale,
                                    const std::string& printer_model, std::string* json_body,
                                    unsigned int* http_code, std::string* http_error, const std::string& provider)
{
    const auto cloud_agent = get_cloud_agent(provider);
    if (cloud_agent)
        return cloud_agent->search_makerworld(query, limit, locale, printer_model, json_body, http_code, http_error);
    return -1;
}

int NetworkAgent::get_makerworld_download_url(const std::string& design_id, std::string* download_url,
                                              std::string* filename, unsigned int* http_code,
                                              std::string* http_error, const std::string& provider)
{
    const auto cloud_agent = get_cloud_agent(provider);
    if (cloud_agent)
        return cloud_agent->get_makerworld_download_url(design_id, download_url, filename, http_code, http_error);
    return -1;
}
