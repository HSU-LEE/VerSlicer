...
public:
    ...
    int search_makerworld(const std::string& query, int limit, const std::string& locale,
                          const std::string& printer_model, std::string* json_body, unsigned int* http_code,
                          std::string* http_error, const std::string& provider = BBL_CLOUD_PROVIDER);
    int get_makerworld_download_url(const std::string& design_id, std::string* download_url, std::string* filename,
                                    unsigned int* http_code, std::string* http_error,
                                    const std::string& provider = BBL_CLOUD_PROVIDER);
