        ...
        if(!has_support() && !m_print->get_no_check_flag()) {
            ...
            if (sntype != NoNeedSupp) {
                std::map<SupportNecessaryType, std::string> reasons = {
                    {SharpTail, L("unsupported areas")},
                    {Cantilever, L("unsupported overhangs")},
                    {LargeOverhang, L("steep overhangs")} };
                std::string warning_message = Slic3r::format(
                    L("Object %1% may have %2%. Try rotating it on the bed or turning on supports."),
