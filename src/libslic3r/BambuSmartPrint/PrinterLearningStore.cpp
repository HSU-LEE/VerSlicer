...
// full_config() keys may be coFloat, coFloats, or coInts — never use opt_float() here.
static void config_set_float_value(DynamicPrintConfig& config, const char* key, float value)
{
    if (!config.has(key))
        return;
    config.set_deserialize_strict(key, std::to_string(int(value)));
}
...
void PrinterLearningStore::apply_learning_to_config(const std::string& printer_id, DynamicPrintConfig& config) const
{
    ...
    if (!category_paused(p, FailureCategory::Adhesion)
        ...
        if (config.has("brim_width")) {
            const float w = std::max(config_get_float(config, "brim_width"), 5.0f);
            config_set_float_value(config, "brim_width", w);
        }
    }
    ...
    if (!setting_adjustment_paused(p, "fan_min_speed")
        ...
        float v = config_get_float(config, "fan_min_speed");
        v += p.setting_adjustments.at("fan_min_speed");
        config_set_float_value(config, "fan_min_speed", std::min(100.f, v));
    }
}
